...
| Split panes | [#483](https://github.com/ensky0/tildaz/issues/483) | Slow track. Steps 1–3 of 6 landed with zero visible change (pixel A/B identical on all three platforms): `src/pane_layout.zig` (pure split tree + per-pane grid, hit testing, neighbor search), the three renderers split into `drawPane(PaneDraw)` / `endFrame`, `SessionCore` tabs became `TabGroup`s over that tree. Step 4a added the `[keys]` pane actions and `SessionCore` operations on every platform; steps 4b–4c wired the Linux host (split / focus / resize / equalize / zoom, gray separators + amber focus line, click-to-focus, separator drag, `…` menu items, `+` Alt-click) and step 5 did the same for macOS and Windows (Windows still awaiting hands-on verification). Step 6 added per-pane IME, a shared drain budget, and per-pane scrollbars, then — after hands-on feedback — trimmed the actions to `split_vertical` / `split_horizontal` (new pane right / below; macOS `alt+cmd`), made separator drag clamp at the minimum size and move only the panes touching the line, and gave the active pane a bracket-shaped mark (three edges when only one edge is shared, all four while zoomed) |
