...
| portal hotkey 와 config mismatch 알림 | ✅ #198 (L9-δ) — portal `BindShortcuts` response 의 `shortcuts a(sa{sv})` 에서 `trigger_description` 추출 → 우리가 보낸 `preferred_trigger` 와 다르면 `org.freedesktop.Notifications.Notify` D-Bus 호출로 사용자에게 desktop notification. dialog (yes/no) 가 아닌 알림만 — Wayland 에 일반 dialog 표준 API 없음 + "프로그램이 config 안 고친다" 정책. 사용자가 보고 `~/.config/tildaz/tildaz.json` 직접 수정 + 재시작. KDE / GNOME / sway / Hyprland / Cinnamon 등 desktop notification daemon 모두 listen. daemon 부재 시 graceful (log only). |
