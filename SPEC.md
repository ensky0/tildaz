...
**Wayland surface 의 논리 크기는 물리 픽셀 격자에 맞춘다** ([#619](https://github.com/ensky0/tildaz/issues/619) 2026-09-04 실측 확정). fractional scale 에서 `논리 × scale / 120` 이 정수가 아니면 우리가 보내는 buffer (`physicalSizeForLogical` 의 반올림값) 와 compositor 가 표면을 놓는 device 사각형이 어긋나 **표면 전체가 리샘플**된다 — 글자까지 흐려진다. 그래서 `computeLayerLayout` 이 dock 의 **논리 크기**를 `120 / gcd(scale, 120)` 논리 px 격자로 맞춘다 (여백은 맞추지 않는다 — 맞추면 `offset_percent` 가 붙이라는 edge 에서 창이 떨어지고, buffer 크기가 목적지와 맞으면 원점의 소수부는 리샘플을 만들지 않는다. 2026-09-04 KDE 실측) (1.25x = 4 · 1.5x = 2 · 1.7x = 10 · 정수 배율 = 1). `height_percent` 가 만드는 소수 논리 높이가 이 규칙이 필요한 이유다 — Hyprland 1.25x 의 논리 1037 은 1296.25 px 라 띠 화면의 셀 경계 14 곳이 전부 다른 밝기로 섞였고, 1036 (= 1295 px) 으로 맞추면 전이 행이 0 이 된다. **전면 점유 (100 %) 축은 맞추지 않는다** — 화면 edge 에 붙는 것이 더 강한 계약이고 그 축의 논리 크기는 compositor 가 알려준 값 하나뿐이다. 논리 화면이 격자 위가 아닌 compositor 도 있어서 (KWin 6.7.4 · 1.7x: 논리 2259x1271 → 2259 × 1.7 = 3840.3) 그 축에도 걸면 창이 edge 에서 떨어진다 (2026-09-04 KDE 실기: width 100 % 인데 오른쪽 15 px 이 남았다).

**그리고 배율이 바뀌면 buffer 크기를 다시 계산한다** (같은 이슈). compositor 는 우리가 요청한 논리 layout 이 그대로면 configure 를 다시 보내지 않으므로 (`height_percent = 100` 이 늘 그렇다), buffer 크기 계산이 configure 핸들러에만 있으면 옛 배율의 물리 크기가 남아 compositor 가 그 buffer 를 새 배율 크기로 늘려 그린다. `applyScaleChange` 가 **마지막 configure 의 논리 크기**로 다시 계산한다 — 실측에서 셀 25 px 로 그린 화면이 31 px 로 나왔다 (1.25× stretch).
...
**어느 쪽으로 가는지는 데스크톱마다 갈린다** (2026-09-03 · 같은 4K 화면 실측 ·
[#583](https://github.com/ensky0/tildaz/issues/583)). KWin 6.7.4 (1.7) · GNOME Shell 50.4 (1.5) ·
sway 1.12 · Hyprland 0.56.2 · cosmic-comp 1.0.0 은 fractional 을 내주고, **Cinnamon 6.6.9 (muffin) 은
내주지 않아 `wl_output` 정수 2.0 으로 간다** — fallback 이 지금도 실제로 쓰이는 경로다. 예전 서술이 mutter ·
wlroots 를 미advertise 예로 든 것은 그 시절 기준이다. GNOME 에서 같은 화면의 정수 scale 은 2 였으므로
fallback 을 탔다면 33 % 크게 그렸을 것이고, Cinnamon 은 데스크톱 자체가 정수 2 (`scaling-factor` auto) 라 맞는다.
...
| 선택 시작 문턱 ([#483](https://github.com/ensky0/tildaz/issues/483) 6단계, 2026-08-28 실기) | 누른 뒤 **문턱보다 많이 움직였거나 다른 칸으로 넘어가야** 선택이 시작된다 (`terminal_interaction.SelectionState.arm` — 세 platform 공통). 문턱은 `ui_metrics.SELECTION_DRAG_SLOP_PT` **4 pt** 을 배율로 옮긴 물리 px 이되 **반 칸을 넘지 않는다** (`selectionDragSlopPx`) — 그래야 아주 작은 폰트에서도 칸 안에서 글자 하나를 끌어 선택할 수 있다. 4 pt 은 Windows 의 드래그 판정 기본값 (`SM_CXDRAG` 4 px @96 dpi) 과 같은 수준. 한 번 넘으면 유지되어 (`armed`) 이웃 칸으로 갔다 돌아오는 한 칸 선택도 된다. **단 누른 자리가 링크면 *다른 칸으로 넘어감* 조건을 쓰지 않고 문턱만 본다** ([#647](https://github.com/ensky0/tildaz/issues/647) — 칸이 좁아 (150 % 에서 14 px) 경계 근처를 누르면 1 px 미끄러짐에도 칸이 바뀌어 선택이 시작되고 링크가 안 열렸다. 칸 경계는 선택의 단위지 클릭과 드래그를 가르는 기준이 아니다. 문턱을 넘으면 그때부터는 평소와 같아 URL 글자를 끌어 고르는 것도 그대로 된다). `on_link` 는 세 host 가 누름 처리에서 이미 계산해 `begin` 에 넘긴다. 그 전에는 트랙패드 클릭의 1~3 px 떨림이 매번 한 칸 선택 + 자동 복사를 만들어, pane 을 클릭해 포커스만 옮겨도 흰 자국이 남고 클립보드가 한 글자로 덮였다 (사용자 캡처 — pane 셋 클릭에 자국 셋) | `App.startTerminalSelection` / `updateTerminalSelection` 이 마우스 px 를 넘긴다 | `tildazMouseDown` / `Dragged` (auto-scroll 은 `g_last_drag_*`) | `pointerPx()` · `selectionSlop()` | ✅ | ✅ | ✅ |
...
- **활성 pane · 탭이 바뀌면 조합을 버린다** ([#536](https://github.com/ensky0/tildaz/issues/536), 2026-08-29). 조합을 시작한 셸이 아닌 곳에서 확정되면 안 된다 (KDE 실기: `^` 를 띄운 채 옆 pane 을 클릭하니 표시가 따라가 그 셸에 `ê` 가 들어갔다). 규칙은 #530 이 창 포커스 이탈에 정한 것과 같다 — IME 는 commit, dead key 는 버림.
    ...
    - **자식 셸이 스스로 끝나서 활성 pane 이 바뀌는 경우 (`drainExitedTabs`) 는 IME preedit 까지 버린다** ([#583](https://github.com/ensky0/tildaz/issues/583) B14 · 2026-09-04 확정). 확정할 대상이 없다 — 그 셸의 PTY 는 이미 닫혔으므로 확정한 바이트는 사라지고, 닫은 *뒤에* 확정하면 형제 pane 의 셸로 샌다 (#536 이 사용자 조작 경로에서 고친 그 오입력이다). 그래서 이 자리만 `leaveShell` 이 아니라 `abandonShell` (= `discardPendingInput` + `resetCompose`) 이다. **활성 pane · 탭이 실제로 바뀔 때만** 부른다 — 백그라운드 탭의 셸이 끝나는 것은 활성 셸을 바꾸지 않으므로 그때 버리면 멀쩡한 조합을 이유 없이 깬다. 이 경로는 사용자 동작이 아니라 PTY 종료가 원인이라 #536 의 handler 목록에 없었고, 그래서 조합이 남아 형제 pane 으로 따라갔다 (코드로 확정: `drainExitedTabs` 가 `leaveShell` 을 부르지 않았다).
...
- **launcher 의 기동 실패 안내도 세 platform 이 다이얼로그다** ([#577](https://github.com/ensky0/tildaz/issues/577)).
  ...
  ([`tool/launcher-fatal-check_windows.ps1`](tool/launcher-fatal-check_windows.ps1) · [#577 댓글](https://github.com/ensky0/tildaz/issues/577)).
...
| ② **용량** | 세 platform 모두 `INITIAL_ATLAS_SIZE` **2048** 에서 시작해 차면 두 배로 키운다 (`MAX_ATLAS_SIZE` **8192** — Linux 는 드라이버 `GL_MAX_TEXTURE_SIZE` 와의 최소). 산술이 아니라 ③ 의 실측으로 정한다 |
...
**키울 때는 다른 줄을 남긴다** (② 의 `grow` — 세 platform).
...
#### ② 계속 — 차면 키운다 (세 platform)

**세 platform 모두 차면 두 배로 키운다** (`GlyphAtlas.grow`). ghostty 가 쓰는 방식이고 (`font.Atlas.grow` + `syncAtlasTexture`), ① 도 완전하지만 구간마다 GPU 를 기다려야 하므로, **차기 전에 키우는 것이 주 경로**다 — ① 은 상한에서만 돈다.
...
| UV | 셰이더가 uniform (`atlas_w`/`atlas_h` · Linux 는 `u_atlas_size`) 으로 정규화하므로 정점의 UV 는 **픽셀 좌표**고, 크기가 바뀌면 그 uniform 과 텍스처를 함께 갱신한다 — macOS 는 `syncAtlasTexture` 가 Metal 텍스처를 다시 만들고, Windows 는 `grow` 가 텍스처 · SRV · D2D render target 체인 일곱 객체를 다시 만들고 옛 내용을 `CopySubresourceRegion` 으로 같은 자리에 복사한다 (CPU 사본이 없다), Linux 는 surface 마다 **CPU shadow** 를 두고 (GLES2 에는 텍스처 간 복사가 없다 — `glCopyImageSubData` 는 3.2, `GL_ALPHA` 는 FBO 첨부 불가) 옛 내용을 새 shadow 에 옮겨 그것으로 새 텍스처를 만든다. Linux 는 전에 UV 를 정점에 미리 나눠 넣어 (`inv_atlas`) 키우면 이미 쌓인 정점이 옛 크기로 굳었다 — #586 에서 uniform 으로 옮겼다 |
...
**Windows 실측** (노트북 LENOVO 83JY · AMD Ryzen AI 7 350 · Radeon 860M · Windows 11 Pro build 26200 · 내장 2880x1800 **120 Hz** · 배율 150 % · Cascadia Code 15pt · `cell 14x29` · 2026-09-03 · [#586](https://github.com/ensky0/tildaz/issues/586)). 같은 방법이다 — `INITIAL_ATLAS_SIZE` 만 바꾼 판을 [`tool/clusters.py`](tool/clusters.py) 의 세 화면으로 띄워 창 캡처를 [`tool/render-ab-shot/render-ab-shot_windows.ps1`](tool/render-ab-shot/render-ab-shot_windows.ps1) 로 견줬다. cell 이 macOS (`19x39`) 보다 작아 **`stack2` 6,000 종이 2048² 에 들어가므로** 기본 판의 `grow` 는 `overflow` (17 화면 누적) 에서 보인다.
...
**Windows · Linux 모두 [#586](https://github.com/ensky0/tildaz/issues/586) 으로 `grow` 를 얹고 실기로 확인했다** — Windows 는 위 실측표 (다섯 판 `0 px` · `atlas full` 0 회), Linux 는 아래 실측표. 검증은 macOS 와 같은 방법 — `INITIAL` 만 바꾼 판을 기준판과 대조 (`tool/clusters.py` · Linux 는 [`tool/render-ab-shot/render-ab-shot_linux.sh`](tool/render-ab-shot/render-ab-shot_linux.sh)). lima VM 은 GL 경로가 켜지지 않아 못 잰다 (AGENTS.md 의 lima 절).
