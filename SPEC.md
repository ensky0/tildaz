...
| `auto_start` | bool | `true` | macOS ServiceManagement `SMAppService.mainAppService` 등록. macOS Login Items / Background Items 에서 TildaZ 가 disallow 되면 앱 실행 전 차단되어 앱 로그도 남지 않는다. v0.4.3 중간 빌드의 legacy `~/Library/LaunchAgents/com.tildaz.app.plist` 는 정리한다. | ✅ | ✅ |
...
| autostart (ServiceManagement) | ✅ | #126, #192 | macOS 13+ 권장 경로인 `SMAppService.mainAppService` 로 main app login item 을 register / unregister 한다. `SMAppService.status` 가 `requiresApproval` 이면 사용자가 System Settings → Login Items & Extensions 에서 직접 허용해야 하며, 앱은 이 토글을 우회해서 켤 수 없다. v0.4.3 중간 빌드에서 만들었던 legacy `~/Library/LaunchAgents/com.tildaz.app.plist` 는 삭제해 다음 로그인부터 중복 등록을 피한다. |
...
*마지막 업데이트: 2026-05-13 (v0.4.3 — macOS Hanja reconversion UX + ServiceManagement autostart + renderer/dialog/env/font-chain 후속 정리).
