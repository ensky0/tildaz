...
| Rename — IME 후보 popup 위치 | cursor 옆 자연 추적 (한자 / kanji / hanzi) | `ImmSetCompositionWindow(CFS_POINT, cursor_pixel)` 매 frame (#164 v0.4.0) | `NSTextInputClient.firstRectForCharacterRange` 가 tab rename snapshot 기준 rect 반환 (#166 v0.4.3) | ✅ | ✅ |
...
| Hanja / kanji reconversion | committed text → 후보 popup → replacement | Win IME native conversion key / candidate popup 경로. app 은 후보 위치를 `ImmSetCompositionWindow` 로 유지 | `NSTextInputClient` reconversion API (`selectedRange`, `markedRange`, `attributedSubstringForProposedRange`, `insertText:replacementRange:`) 구현. terminal cursor row / tab rename 범위만 safe replacement, 그 외 범위는 안전하게 plain insert fallback (#166 v0.4.3) | ✅ | ✅ |
...
*마지막 업데이트: 2026-05-11 (v0.4.3 — macOS Hanja reconversion + renderer/dialog/env/font-chain 후속 정리).
