...
usage() {
    cat <<'USAGE'
쓰는 법: tool/stress/check-input-loss_linux_macos.sh [옵션]
...
. "$REPO_ROOT/tool/stress/hygiene.sh"
...
⚠ **②를 하려면 입력기(IME)가 영문이어야 해요.** 한글 모드면 타이핑이 한글로 조합돼
   `touch` 가 명령이 되지 않아요. ①은 한글 모드에서도 정상이에요 (fcitx5 · KDE 실측).
...
HINT
elif [ "$SNAPS" -gt 0 ] && [ "$PTY_OK" -eq 0 ]; then
    cat >&2 <<'HINT'
⚠ ①은 됐는데 ②만 실패했으면 **입력기(IME)가 한글 모드였을 가능성이 큽니다.**
  타이핑이 한글로 조합돼 `touch` 가 명령이 되지 않아요. ①은 한글 모드에서도 정상이라
  이 조합이 IME 의 표시입니다 (#441 실측: fcitx5 · KDE).
HINT
elif [ "$SNAPS" -eq 0 ]; then
    cat >&2 <<'HINT'
⚠ 폭포는 흘렀는데 ①이 0 이면 키가 앱까지 오지 않은 거예요.
  **창에 키보드 포커스가 있었는지** 먼저 확인하세요 — 스크립트가 앱을 띄운 뒤에도
  포커스가 원래 터미널에 남아 있을 수 있어요. (#441 에서 한 회차가 0/10 이었는데
  재현되지 않았고, 그때 누른 키는 어느 앱 로그에도 남지 않았어요 — 원인 미확정.)
HINT
