...
| portal hotkey 와 config 자동 적용 (#207, 이전 #198 의 L9-δ Option Y 갱신) | 🟨 코드 구현 완료, 사용자 시연 검증 대기. **정책 변경 (#198 → #207)**: 이전 결정 (Option Y, *notification 만 + 사용자 수동 config 수정*) 의 *전제* = *우리 own dialog backend 없었음*. #203 Phase C step 3/4 로 layer-shell overlay dialog 가용 → 정책 재검토. 새 정책 — **config = source of truth + system binding → config 값으로 자동 update + 우리 dialog 안내** (mac/win 의 *config 우선* 정책 동등). 구현: 시작 시 portal `BindShortcuts` response 의 `actual_trigger` 와 우리 `preferred_trigger` 다르면 `portal.handleHotkeyMismatch` 의 **계층적 fallback chain**: <br/>① 1차 (cross-DE 정공): XDG portal `UnbindShortcuts` (spec v2 since) + 재 `BindShortcuts`. <br/>② 2차 (DE-specific): 1차 실패 시 `XDG_CURRENT_DESKTOP` 감지 + DE native API. **KDE**: `org.kde.kglobalaccel.unregister(component="tildaz", shortcut="toggle")` D-Bus call → kglobalshortcutsrc 의 `[tildaz]` group binding 삭제 → 재 `BindShortcuts` 시 *처음 등록* path 로 새 trigger 적용. **GNOME / sway / hyprland**: 별 후속 작업 (#207 미구현). <br/>③ 3차 fallback: 모든 path 실패 / 미지원 DE → `dialog.showInfo` (layer-shell overlay) 로 *수동 변경 안내*. <br/>**사용자 시연 발견 (#207 진행 중)**: KDE Plasma 6.6.5 의 portal-kde 가 `UnbindShortcuts` 미구현 (`"No such method"` D-Bus error) → 1차 항상 fail → 2차 KDE D-Bus path 가 *실제 동작 경로*. portal-kde upstream 의 v2 추가 후 1차 cross-DE 정공으로 동작 예정. |
