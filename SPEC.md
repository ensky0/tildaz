...
| 선택 시작 문턱 — **누른 셀을 벗어나야 선택** ([#483](https://github.com/ensky0/tildaz/issues/483) 6단계, 2026-08-28 실기) | 공통 `terminal_interaction.SelectionState.arm` — 셀 기준이라 배율 · DPI 무관. 한 번 벗어나면 유지되어 이웃 칸으로 갔다 돌아오는 한 칸 선택은 그대로 된다. 그 전에는 트랙패드 클릭의 1 px 떨림이 매번 한 칸 선택 + 자동 복사를 만들어, pane 을 클릭해 포커스만 옮겨도 흰 자국이 남고 클립보드가 한 글자로 덮였다 (사용자 캡처 — pane 셋 클릭에 자국 셋) | 동일 | 동일 | 동일 | 🔶 | 🔶 | 🔶 |
...
탭 하나 = pane 그룹 (`session_core.TabGroup`), pane = 터미널 하나 (`Tab`). 배치는 분할 트리 (`pane_layout.Tree`) 이고 격자 계산 · hit-test · 이웃 찾기는 순수 모듈 [`pane_layout.zig`](src/pane_layout.zig) 한 곳이다. 확정 설계는 이슈 첫 댓글, 단축키 표는 [KEYBINDINGS.md](KEYBINDINGS.md#split-panes). 2026-08-27 4단계에 Linux, 5단계에 macOS · Windows host 를 배선했다. 표의 🔶 = 구현은 들어갔으나 실기 검증 대기 (Windows 는 다른 머신에서 확인한다). 2026-08-27 6단계 마무리에서 사용자 결정으로 단축키 (분할 둘로 축소 · macOS `alt+cmd`) · 표시 (3 면 규칙 · 최대화 4 면) · 크기 조절 (clamp · 붙은 칸만) 을 바꿨다 — 바뀐 행은 macOS (자동 조작 + 캡처 픽셀 판독) · Linux (VM 헤드리스) 로 다시 확인했고 Windows 만 🔶 (Linux 분할선 드래그는 마우스 경로를 다시 안 돌려 🔶 — 같은 `setSeparatorPx` 를 키 경로로 확인).
