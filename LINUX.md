...
| 한글 IME 입력 (fcitx5 + Cinnamon Wayland) | 동작 — `zwp_text_input_v3` wire-level 구현 + commit_string event → PTY 송신 + preedit inline overlay (보라색 배경, macOS / Windows 동등). spec done-apply batch 패턴. L10-α + L10-β. |
...
우선순위가 높은 순서. 6c685b6 까지 완료된 항목은 ✅ 표시.
