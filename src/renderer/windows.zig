...
pub const D3d11Renderer = struct {
    ...
    pub fn drawPane(self: *D3d11Renderer, pane: pane_draw.PaneDraw) void {
        ...
        for (0..rows) |y| {
            ...
            while (x < cols) {
                ...
                // 과 같은 이유로 procedural 사각형 (#258). 폰트 글리프는 cell 에 안
                // 맞아 셀 사이 갭이 생기고, powerline 은 기본 폰트 묶음에 아예 없어
                // 물음표로 나왔다. 대상 범위는 `box_drawing.handles` 하나다.
    ...
    fn resolveBg(style: ghostty.Style, raw: *const ghostty.Cell, colors: *const ghostty.RenderState.Colors, is_selected: bool, is_inverse: bool, hl: ?cell_color.HighlightColors, dbg_r: f32, dbg_g: f32, dbg_b: f32) [3]f32 {
