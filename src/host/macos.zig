...
/// #483 5단계 — pane 격자 계산 입력 (Linux `Client.paneMetrics` 상당). `terminalGrid` 와 같은 반올림
/// (`scaledPx`) 에 분할선 두께를 더한 것.
fn paneMetricsMac() pane_layout.Metrics {
    const r = &g_renderer.?;
    return .{
        .cell_w = @intCast(r.font.cell_width_px),
        .cell_h = @intCast(r.font.cell_height_px),
        .pad = ui_metrics.scaledPx(i32, TERMINAL_PADDING_PT, r.scale),
        .scrollbar_w = ui_metrics.scaledPx(i32, ui_metrics.SCROLLBAR_W_PT, r.scale),
        .separator_w = @intFromFloat(ui_metrics.linePx(ui_metrics.PANE_SEPARATOR_W_PT, r.scale)),
    };
}

/// #483 5단계 — 탭바를 뺀 터미널 영역 (`pane_layout.layout` 의 `rect`).
fn paneAreaMac() pane_layout.Rect {
    const r = &g_renderer.?;
    const tab_bar = tabBarHeightPx(r.scale);
    return .{ .x = 0, .y = tab_bar, .w = @intCast(r.vp_width), .h = @as(i32, @intCast(r.vp_height)) - tab_bar };
}

/// #483 5단계 — 활성 탭의 pane 배치 (최대화 반영, `TabGroup.layout`). renderer 나 탭이 없으면 빈 slice.
fn activeLayoutMac(buf: *[pane_layout.MAX_PANES_PER_TAB]pane_layout.PaneRect) []pane_layout.PaneRect {
    if (g_renderer == null) return buf[0..0];
    const group = g_session.activeGroup() orelse return buf[0..0];
    return group.layout(paneAreaMac(), paneMetricsMac(), buf);
}

/// 활성 pane 의 배치 (영역 · 격자 · 격자 원점). 없으면 null.
fn activePaneRectMac() ?pane_layout.PaneRect {
    if (g_renderer == null) return null;
    const group = g_session.activeGroup() orelse return null;
    var buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.PaneRect = undefined;
    return pane_layout.find(activeLayoutMac(&buf), group.active_pane);
}

/// 활성 pane 의 첫 셀 좌상 px (f32 — 이 host 의 hit-test 좌표가 f32 다). pane 이 없으면 pane 하나일
/// 때의 값 (`pad` · `tab_bar_h + pad`).
fn activeGridOriginPx() struct { x: f32, y: f32 } {
    if (activePaneRectMac()) |pr| return .{ .x = @floatFromInt(pr.grid_x), .y = @floatFromInt(pr.grid_y) };
    const scale: f32 = if (g_renderer) |r| r.scale else 1.0;
    const pad: f32 = @floatFromInt(ui_metrics.scaledPx(i32, TERMINAL_PADDING_PT, scale));
    return .{ .x = pad, .y = @as(f32, @floatFromInt(tabBarHeightPx(scale))) + pad };
}

/// #483 5단계 — pane 의 scrollbar track inset. 단일 탭 컨트롤 스트립 (#329) 은 오른쪽 위에 얹히므로
/// 그 모서리를 가진 pane 만 (pane 하나면 이전의 `scrollbarTopInsetPx − tab_bar` 와 같은 값).
fn paneScrollbarTopInsetMac(rect: pane_layout.Rect, area: pane_layout.Rect) i32 {
    const scale = g_renderer.?.scale;
    const inset = scrollbarTopInsetPx(scale) - tabBarHeightPx(scale);
    if (inset <= 0) return 0;
...
/// 중엔 `renderFrameTick` 이 트리 복사본에 놓아 본 자리에 amber 고스트를 그린다.
const SepDrag = struct { node: u8, axis: pane_layout.Axis, px: i32 };
var g_sep_drag: ?SepDrag = null;

/// 포인터 아래의 분할선 (양쪽 `PANE_SEPARATOR_HIT_SLOP_PT`). 메뉴가 떠 있으면 null.
fn separatorAtMac(x: f32, y: f32) ?pane_layout.Separator {
    if (g_renderer == null or g_command_menu_open) return null;
    const group = g_session.activeGroup() orelse return null;
    var buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.Separator = undefined;
    const seps = group.separators(paneAreaMac(), paneMetricsMac(), &buf);
    const slop = ui_metrics.scaledPx(i32, ui_metrics.PANE_SEPARATOR_HIT_SLOP_PT, g_renderer.?.scale);
    return pane_layout.separatorAt(seps, @intFromFloat(@floor(x)), @intFromFloat(@floor(y)), slop);
}

/// 포인터가 활성 pane 의 scrollbar 자리 (오른쪽 `scrollbar_w`, pane 높이 전체) 안인가. pane 하나면
/// 이전의 `x >= vp_w − sbw` 와 같다.
fn inActiveScrollbarColumnMac(x: f32, y: f32) bool {
    const pr = activePaneRectMac() orelse return false;
    const sbw = ui_metrics.scaledPxF(ui_metrics.SCROLLBAR_W_PT, g_renderer.?.scale);
    const right: f32 = @floatFromInt(pr.rect.x + pr.rect.w);
    const top: f32 = @floatFromInt(pr.rect.y);
    const bottom: f32 = @floatFromInt(pr.rect.y + pr.rect.h);
    return x >= right - sbw and x < right and y >= top and y < bottom;
}
...
fn syncTerminalGeometry() void {
    ...
    // #483 5단계 — pane 마다 격자가 다르다. 창 · 탭바 · metrics 로 layout 을 펴서 모든 탭의 pane 에 준다
    // (`applyLayouts` 는 같은 격자면 건너뛴다). pane 하나면 `terminalGrid` 와 같은 값이다.
...
fn sendEncodedKeyMac(tab: anytype, event: objc.id, utf8: []const u8, action: key_encode.Action) bool {
    ...
    if (bytes.len == 1 and bytes[0] == 0x03) {
        tab.interruptWrite(bytes);
        // #242 — Ctrl+C 도 사용자 입력이라 맨 아래로 (^C 와 새 prompt 가 보이게).
        tab.terminal.scrollViewport(.{ .bottom = {} });
    } else {
        writeUserInput(tab, bytes);
    }
    return true;
}
...
fn tildazFlagsChanged(_: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    if (event == null) return;
    // #647 — `⌘` 를 누르거나 뗀 그 순간 링크 판정을 다시 한다. 마우스가 가만히 있으면
    // motion 이 오지 않아서, 이것이 없으면 포인터를 흔들어야 밑줄 · 손 커서가 따라온다
    // (2026-09-11 사용자 제안). 앱이 마우스를 잡은 동안에만 실제로 달라진다 — 평소에는
    // `active` 가 수식키와 무관해 `needsUpdate` 가 걸러 낸다.
    refreshLinkHoverMac();
    if (g_command_menu_open) return;
    if (g_marked_len > 0) return;
    const tab = g_session.activeTab() orelse return;
...
fn tildazKeyDown(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    ...
    if (g_marked_len == 0) {
        ...
        // #533 — 특수키를 인코더로 보낸다. 예전엔 modifier 를 싣지 않는 고정 표
        // (`keyCodeToEscape`) 라 Shift+← · Alt+← 가 아무것도 내지 않았다. 다루는 키
    ...
    if (g_marked_len > 0) {
        ...
        if (physical_key.fromMacKeyCode(keycode2)) |code| {
...
fn fillImeSnapshot(allocator: std.mem.Allocator, snap: *ImeSnapshot) !bool {
    ...
    // #483 5단계 — 활성 pane 의 격자 원점 (pane 하나면 `pad` · `tab_bar_h + pad`).
    const origin = activeGridOriginPx();
    const grid_x: f32 = origin.x;
    const row_y = origin.y + @as(f32, @floatFromInt(vp.y)) * ch;
...
fn eventToCell(self_view: objc.id, event: objc.id) ?terminal_interaction.Cell {
    ...
    return cellFromPx(px.x, px.y);
}

/// `eventToCell` 의 본체 — 좌표만 받는다. **키 이벤트에는 마우스 좌표가 없어서** 링크 판정이
/// 마지막 포인터 위치 (`g_link_pointer_*`) 로 같은 계산을 해야 한다 (#647).
fn cellFromPx(px_x: f32, px_y: f32) ?terminal_interaction.Cell {
    if (g_renderer == null) return null;
    const tab = g_session.activeTab() orelse return null;
    ...
    // 격자 원점 앞 (padding · 탭바) 은 cell 이 아니다 (탭바 클릭은 별도 dispatch).
    if (px_x < origin.x or px_y < origin.y) return null;
    const x_in: f32 = px_x - origin.x;
    const y_in: f32 = px_y - origin.y;
...
fn cellAndDirFromPx(x: f32, y: f32) ?struct { cell: terminal_interaction.Cell, dir: i8 } {
    ...
    const cell_w_px = g_renderer.?.font.cell_width_px;
    const cell_h_px = g_renderer.?.font.cell_height_px;
    // #483 5단계 — 활성 pane 의 격자 원점.
    const origin = activeGridOriginPx();
    const col_i: i32 = @floor((x - origin.x) / @as(f32, @floatFromInt(cell_w_px)));
    const row_i: i32 = @floor((y - origin.y) / @as(f32, @floatFromInt(cell_h_px)));
    return .{
...
fn reportGeometryMac() ?terminal_interaction.ReportGeometry {
    if (g_renderer == null) return null;
    const tab = g_session.activeTab() orelse return null;
    // #483 5단계 — 격자 원점은 활성 pane 의 것.
    const pr = activePaneRectMac() orelse return null;
    return .{
...
/// 마지막으로 본 포인터 위치 (물리 px). **키 이벤트에는 마우스 좌표가 없어서** `⌘` 를 눌렀을
/// 때 "지금 포인터가 링크 위인가" 를 이 값으로 판정한다 (2026-09-11 사용자 제안).
var g_link_pointer_x: f32 = 0;
var g_link_pointer_y: f32 = 0;
var g_link_pointer_valid: bool = false;

/// 지금 `⌘` 가 눌려 있는가. `NSEvent` 의 클래스 메서드라 **이벤트 없이도** 읽을 수 있다 —
/// 키 이벤트 · 마우스 이벤트 어디서든 같은 값이다.
fn linkModsNowMac() bool {
    const NSEvent = objc.getClass("NSEvent");
    const get_flags = objc.objcSend(fn (objc.Class, objc.SEL) callconv(.c) c_ulong);
    return (get_flags(NSEvent, objc.sel("modifierFlags")) & (1 << 20)) != 0;
}

/// 포인터 아래에 맞는 커서를 **즉시** 지정한다.
///
/// **`addCursorRect:cursor:` 로는 안 된다** — AppKit 은 마우스가 그 사각형에 *들어올 때* 커서를
/// 바꾸는데, hover 중에는 포인터가 이미 그 자리에 있어서 rect 를 다시 만들어도 (그래서
/// `invalidateCursorRects` 를 불러도) 진입 이벤트가 없다. 2026-09-11 실기에서 밑줄은 뜨는데
/// 손 커서만 안 나온 원인이 이것이었다.
///
/// 그래서 링크 위에서는 `NSCursor.set` 으로 직접 지정하고, 벗어나면 셀 영역의 기본값
/// (I-beam) 으로 되돌린다. 셀 밖 (탭바 · padding · 스크롤바) 은 건드리지 않는다 — 그쪽은
/// `tildazResetCursorRects` 의 rect 가 이미 맞게 잡아 준다.
fn applyLinkCursorMac() void {
    const NSCursor = objc.getClass("NSCursor");
    const get = objc.objcSend(fn (objc.Class, objc.SEL) callconv(.c) objc.id);
    const set = objc.objcSend(fn (objc.id, objc.SEL) callconv(.c) void);

    if (g_link_hover.hit != null) {
        if (get(NSCursor, objc.sel("pointingHandCursor"))) |hand| set(hand, objc.sel("set"));
        return;
    }
    // 링크를 벗어났다 — 셀 위면 I-beam 으로 되돌린다.
    if (!g_link_pointer_valid) return;
    if (cellFromPx(g_link_pointer_x, g_link_pointer_y) == null) return;
    if (get(NSCursor, objc.sel("IBeamCursor"))) |ibeam| set(ibeam, objc.sel("set"));
}

/// 마지막 포인터 위치로 링크 판정을 다시 한다. `⌘` 를 누르거나 뗀 순간에 부른다 — 그때
/// 마우스는 가만히 있으므로 motion 이 오지 않는다.
fn refreshLinkHoverMac() void {
    if (!g_link_pointer_valid) return;
    const tab = g_session.activeTab() orelse return;
    const cell = cellFromPx(g_link_pointer_x, g_link_pointer_y);
    const probe: link.Hover.Probe = .{
    ...
    // 커서는 **바뀌지 않았어도 매번** 지정한다 — AppKit 이 cursor rect 평가로 I-beam 을
    // 되돌릴 수 있어서, 한 번만 set 하면 다음 motion 에 사라진다.
    applyLinkCursorMac();
}
...
fn updateLinkHoverMac(self_view: objc.id, event: objc.id) void {
    ...
    // 키 이벤트가 나중에 같은 자리를 다시 판정할 수 있게 좌표를 기억한다.
    if (event != null) {
        const px = eventToWindowPx(self_view, event);
        g_link_pointer_x = px.x;
        g_link_pointer_y = px.y;
        g_link_pointer_valid = true;
    }

    const probe = linkProbeMac(self_view, event);
    // **커서는 판정이 그대로여도 매번 다시 지정한다.** 같은 셀 안에서 마우스가 움직이면
    // `needsUpdate` 가 false 라 아래에서 빠져나가는데, 그 사이 AppKit 이 cursor rect 평가로
    // I-beam 을 되돌린다 — 2026-09-11 실기에서 손 커서가 셀 경계를 넘을 때만 한 순간
    // 떴다 사라진 원인이 이것이었다.
    if (!g_link_hover.needsUpdate(probe)) {
        applyLinkCursorMac();
        return;
    }
...
/// 그래서 이것만 보면 "보이는 것과 되는 것" 이 저절로 맞는다.
fn linkUrlMac(self_view: objc.id, event: objc.id) ?[]const u8 {
    updateLinkHoverMac(self_view, event);
    const cell = eventToCell(self_view, event) orelse return null;
    return g_link_hover.urlAt(.{ .x = cell.col, .y = cell.row });
}
...
fn singleControlHit(px: f32, py: f32) TabBarArea {
    if (g_renderer == null or g_session.count() != 1) return .none;
    const h: f32 = @floatFromInt(scrollbarTopInsetPx(g_renderer.?.scale));
    return switch (tab_layout.hitArea(px, py, h, tabBarLayout())) {
        .plus, .close, .more => |a| a,
        else => .none,
    };
}
...
/// 호출 전 `g_renderer != null` 확인 필수.
fn commandMenuViewNow() command_menu.View {
    const r = &g_renderer.?;
    return command_menu.view(
        @as(f32, @floatFromInt(r.vp_width)) / r.scale,
        @as(f32, @floatFromInt(r.vp_height)) / r.scale,
        @floatFromInt(ui_metrics.TAB_BAR_HEIGHT_PT),
        g_command_menu_first,
    );
}

/// #329 — 메뉴 닫기 공통 지점. focus / scroll 까지 초기화해 다음 열기가
/// 항상 처음 상태에서 시작한다.
fn closeCommandMenu() void {
...
fn commandMenuHit(px: f32, py: f32) ?command_menu.Command {
    if (!g_command_menu_open or g_renderer == null) return null;
    const scale = g_renderer.?.scale;
    return command_menu.hit(commandMenuViewNow(), px / scale, py / scale);
}
...
fn handleCommandMenuKey(menu_key: command_menu.MenuKey) void {
    switch (command_menu.onKey(menu_key, &g_command_menu_focus)) {
        .consumed => {
            // #334 — 키보드를 쓰는 순간 pointer hover 강조를 지운다. 마우스가
            // 항목 위에 머물러 있으면 hover 가 focus 를 덮어 키보드 이동이
            // 화면에 안 보였다 (사용자 시연 발견). 마우스를 다시 움직이면
            // hover 가 재적용되며 focus 도 동기화된다.
            g_command_menu_hover = null;
            if (g_command_menu_focus) |focused| {
                if (g_renderer != null) {
                    const r = &g_renderer.?;
                    g_command_menu_first = command_menu.ensureVisible(
                        @as(f32, @floatFromInt(r.vp_width)) / r.scale,
                        @as(f32, @floatFromInt(r.vp_height)) / r.scale,
                        @floatFromInt(ui_metrics.TAB_BAR_HEIGHT_PT),
                        g_command_menu_first,
        ...
        .activate => |command| {
            // keyboard 실행도 outside click 과 동일 — pending 입력 commit 후.
            if (g_window != null) {
                const contentView_get = objc.objcSend(fn (objc.id, objc.SEL) callconv(.c) objc.id);
                const cv = contentView_get(g_window, objc.sel("contentView"));
                if (cv != null) commitPendingInput(cv);
            }
            executeCommandMenu(command);
        },
    }
}
...
fn tildazResetCursorRects(self_view: objc.id, _: objc.SEL) callconv(.c) void {
    ...
    const arrow = get_ibeam(NSCursor, objc.sel("arrowCursor"));
    const add_rect = objc.objcSend(fn (objc.id, objc.SEL, NSRect, objc.id) callconv(.c) void);

    // #483 5단계 — pane 마다 셀 영역 (Y-up flip): x=[rect.x+pad, rect.right−pad−sbw], y 는 pane 의 위·아래
    // padding 안쪽. pane 하나면 이전의 `[pad, w−pad−sbw] × [pad, h−tab_bar_h−pad]` 와 같다. 분할선 위는
    // 리사이즈 커서 (±slop) — 셀 rect 뒤에 얹어 우선한다.
    if (g_renderer != null) {
        const s: f64 = @floatCast(g_renderer.?.scale);
        var rect_buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.PaneRect = undefined;
        for (activeLayoutMac(&rect_buf)) |pr| {
            const rx: f64 = @as(f64, @floatFromInt(pr.rect.x)) / s;
            const ry: f64 = @as(f64, @floatFromInt(pr.rect.y)) / s;
            const rw: f64 = @as(f64, @floatFromInt(pr.rect.w)) / s;
            const rh: f64 = @as(f64, @floatFromInt(pr.rect.h)) / s;
            const cell_w = rw - 2.0 * pad - sbw;
            const cell_h = rh - 2.0 * pad;
            if (cell_w <= 0 or cell_h <= 0) continue;
            const cell_rect = NSRect{
                .origin = .{ .x = rx + pad, .y = h - (ry + rh) + pad },
                .size = .{ .width = cell_w, .height = cell_h },
            };
            add_rect(self_view, objc.sel("addCursorRect:cursor:"), cell_rect, ibeam);
        }
        const lr = get_ibeam(NSCursor, objc.sel("resizeLeftRightCursor"));
        const ud = get_ibeam(NSCursor, objc.sel("resizeUpDownCursor"));
        const slop: f64 = @floatFromInt(ui_metrics.PANE_SEPARATOR_HIT_SLOP_PT);
        var sep_buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.Separator = undefined;
        if (g_session.activeGroup()) |group| {
            for (group.separators(paneAreaMac(), paneMetricsMac(), &sep_buf)) |sp| {
                const cursor = if (sp.axis == .side_by_side) lr else ud;
                if (cursor == null) continue;
                const sx: f64 = @as(f64, @floatFromInt(sp.rect.x)) / s;
                const sy: f64 = @as(f64, @floatFromInt(sp.rect.y)) / s;
                const sw: f64 = @as(f64, @floatFromInt(sp.rect.w)) / s;
                const sh: f64 = @as(f64, @floatFromInt(sp.rect.h)) / s;
                const sep_rect = if (sp.axis == .side_by_side)
                    NSRect{ .origin = .{ .x = sx - slop, .y = h - (sy + sh) }, .size = .{ .width = sw + 2 * slop, .height = sh } }
                else
                    NSRect{ .origin = .{ .x = sx, .y = h - (sy + sh) - slop }, .size = .{ .width = sw, .height = sh + 2 * slop } };
                add_rect(self_view, objc.sel("addCursorRect:cursor:"), sep_rect, cursor);
            }
        }
    }

    // 단일 탭 control strip과 열린 command menu는 terminal 위 overlay라 cell
    // I-beam rect와 겹친다. 뒤에 arrow rect를 추가해 버튼 의미를 우선한다.
    if (arrow != null and g_session.count() == 1) {
        const controls_w: f64 = @floatFromInt(ui_metrics.TAB_PLUS_W_PT + ui_metrics.TAB_CLOSE_W_PT + ui_metrics.TAB_MORE_W_PT);
        const controls = NSRect{
            .origin = .{ .x = @max(0, w - controls_w), .y = @max(0, h - @as(f64, @floatFromInt(TAB_BAR_HEIGHT_PT))) },
            .size = .{ .width = @min(w, controls_w), .height = @min(h, @as(f64, @floatFromInt(TAB_BAR_HEIGHT_PT))) },
        };
    ...
    if (arrow != null and g_command_menu_open and g_renderer != null) {
        ...
        const menu_rect = NSRect{
            .origin = .{ .x = menu.x, .y = @max(0, h - @as(f64, menu.y + menu.h)) },
            .size = .{ .width = menu.w, .height = @min(h, @as(f64, menu.h)) },
        };
...
fn tildazMouseDown(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    ...
    if (event == null) return;
    // #483 5단계 — 누름이 다른 pane 으로 포커스를 옮기면 아래에서 다시 잡는다.
    var tab = g_session.activeTab() orelse return;
    ...
    if (g_command_menu_open) {
        ...
        if (g_renderer != null) {
            const menu_view = commandMenuViewNow();
            const s = g_renderer.?.scale;
            if (command_menu.hitScrollIndicator(menu_view, xy.x / s, xy.y / s)) |dir| {
                g_command_menu_first = command_menu.scrollStep(menu_view, dir == .down);
                requestRender();
                return;
            }
        }
        const hit = commandMenuHit(xy.x, xy.y);
        closeCommandMenu();
    ...
    if (g_session.count() == 1 and g_renderer != null) {
        const xy = eventToWindowPx(self_view, event);
        switch (singleControlHit(xy.x, xy.y)) {
            .plus => {
                tab.interaction.cancelPointerModes();
    ...
    if (g_session.count() >= 2 and g_renderer != null) {
        ...
        switch (hit_area) {
            ...
            .plus => {
                ...
                if (layout.plus_enabled) handlePlusClick(event);
                return;
            },
    ...
    if (g_renderer != null) {
        const xy = eventToWindowPx(self_view, event);
        // #483 5단계 — 분할선 (±slop) 누름 = 드래그 시작 (pane 판정보다 먼저 — Linux 와 같은 순서).
        if (separatorAtMac(xy.x, xy.y)) |s| {
            tab.interaction.cancelPointerModes();
            g_drag.reset();
            g_sep_drag = .{ .node = s.node, .axis = s.axis, .px = @intFromFloat(@floor(if (s.axis == .side_by_side) xy.x else xy.y)) };
            return;
        }
        // 다른 pane 을 눌렀으면 먼저 그 pane 으로 포커스 (preedit 은 위에서 확정했다) 하고, 그 pane 기준으로
        // 아래 scrollbar · 셀 판정을 이어 간다 (포커스 이동과 선택 시작이 한 클릭).
        if (focusPaneUnderPointer(xy.x, xy.y)) tab = g_session.activeTab() orelse return;
        if (inActiveScrollbarColumnMac(xy.x, xy.y)) {
    ...
    // #647 — 링크 위 누름은 **앱에 보내지 않는다** (이 클릭은 링크 것이다). 열기는 뗌에서
    // 한다 — 드래그로 선택한 경우와 갈라야 해서다. 아래 selection 은 그대로 시작하므로
    // 링크 위에서 드래그해 글자를 고르는 것도 된다.
    const on_link = linkUrlMac(self_view, event) != null;
...
fn tildazMouseDragged(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    ...
    if (g_sep_drag) |*d| {
        const xy = eventToWindowPx(self_view, event);
        d.px = @intFromFloat(@floor(if (d.axis == .side_by_side) xy.x else xy.y));
        return;
    }
...
fn tildazMouseUp(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    ...
    // #647 — 드래그로 고르지 않은 **순수 클릭**이 링크 위면 연다. 뗌에서 하는 이유가
    // 이것이다 — 누름에서 열면 링크 위에서 글자를 끌어 고를 수 없다. 앱에는 보내지 않는다
    // (누름도 안 보냈다).
    {
        const plain_click = tab.interaction.selection.active and !tab.interaction.selection.armed;
        if (plain_click) {
            if (linkUrlMac(self_view, event)) |url| {
...
fn tildazMouseMoved(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    ...
    if (g_command_menu_open) {
        const xy = eventToWindowPx(self_view, event);
        const hover = commandMenuHit(xy.x, xy.y);
        if (hover != g_command_menu_hover or g_tab_hover != .none) {
            g_command_menu_hover = hover;
            // #329 — pointer 가 항목 위로 오면 keyboard focus 도 동기화 (표준
            // 메뉴 동작). 안 하면 마우스로 건너뛴 뒤 ↑↓ 가 옛 위치에서 출발.
            if (hover) |h| g_command_menu_focus = h;
            g_tab_hover = .none;
            requestRender();
        }
        return;
...
/// (변경 후) 와 동일.
fn tildazRightMouseDown(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    // #329 — 열린 menu 는 pointer button 보다 우선: 우클릭은 메뉴만 닫고
    // paste 하지 않는다 (SPEC §5.3).
    if (g_command_menu_open) {
        closeCommandMenu();
        return;
    }
    ...
    applyPasteInputPolicy();
    // #483 5단계 — 비활성 pane 우클릭은 포커스만 옮기고 붙여넣지 않는다 (확정 설계 축 3).
    if (event != null) {
        const xy = eventToWindowPx(self_view, event);
...
fn scrollbarHit() ?scrollbar.Hit {
    ...
    return scrollbar.hit(
        ...
        @floatFromInt(pr.rect.y + paneScrollbarTopInsetMac(pr.rect, paneAreaMac())),
...
/// 호출처가 preedit 을 **먼저** 확정했다 (`tildazMouseDown` 첫 줄 · 우클릭의 `applyPasteInputPolicy`) —
/// 순서가 바뀌면 자모가 새 pane 의 PTY 로 간다.
fn focusPaneUnderPointer(x: f32, y: f32) bool {
    if (g_renderer == null) return false;
    const group = g_session.activeGroup() orelse return false;
    const id = g_session.paneIdAt(@intFromFloat(@floor(x)), @intFromFloat(@floor(y)), paneAreaMac(), paneMetricsMac()) orelse return false;
...
fn tildazScrollWheel(self_view: objc.id, _: objc.SEL, event: objc.id) callconv(.c) void {
    ...
    // #329 — 열린 menu 는 wheel 도 소비한다. 작은 viewport 에서 잘린 항목
    // 도달 경로 (entry 단위 scroll). terminal scrollback 으로 안 보냄.
    // #334 — delta 누적 + 나머지 보존: 이벤트당 1 entry 는 트랙패드(초당
    // 수십 개의 작은 delta)에서 과민했다 (재감사 발견). 12pt 당 1 entry 는
    // 시작값 — 시연 튜닝.
    if (g_command_menu_open) {
        if (g_renderer == null) return;
        ...
        while (steps != 0) {
            ...
            const next = command_menu.scrollStep(commandMenuViewNow(), down);
            if (next == g_command_menu_first) break;
            g_command_menu_first = next;
            requestRender();
...
pub fn run(rt: Runtime, opts: run_options.RunOptions) !void {
    ...
    // 멈추면 안 되므로 로그로만 남긴다.
    config.showConfigNotice(rt, g_run_opts.isStressRun());
...
fn renderFrameTick() void {
    ...
    const group = g_session.activeGroup().?;
    const area = paneAreaMac();
    const m = paneMetricsMac();
    var rect_buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.PaneRect = undefined;
    ...
    for (lay) |pr| {
        ...
        g_renderer.?.drawPane(.{
            ...
            .scrollbar_top_inset = paneScrollbarTopInsetMac(pr.rect, area),
    ...
    // 분할선 · amber · 드래그 고스트 (Linux 와 같은 규칙). 고스트는 트리 복사본에 놓아 본 자리 —
    // 스냅 · 최소 크기 판정을 실제 놓기와 같은 함수가 한다.
    var sep_buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.Separator = undefined;
