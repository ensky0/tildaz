...
#
#   tool/headless-check_linux.sh up                  # headless sway 1600x1000 + vkbd 데몬 (한 번)
#   tool/headless-check_linux.sh tabs                # A8  — Alt+1~9 탭 전환 (탭마다 cat > tab_N.txt · 파일로 판정)
#   tool/headless-check_linux.sh confirm             # A7  — Alt+F4 확인 다이얼로그 펌프 중 SIGTERM (#521)
#   tool/headless-check_linux.sh prompt              # A7  — 새 instance 핫키 캡처 다이얼로그 펌프 중 SIGTERM (#521)
#   tool/headless-check_linux.sh first-run           # #620 — config 없는 첫 실행에서 창 안 단축키가 먹는지
#   tool/headless-check_linux.sh scale               # A5  — 배율 1.25 · 1.5 · 2.0 · 1.7 의 띠 화면 전이 행 (#539)
...
print(f'logical {$rw}x{h} × {s:.4f} = {v:.2f} → buffer 반올림 {ra} / 내림 {int(v)} · 소수부 {round(v%1,3)}')")
    ...
    python3 "$ROOT/tool/bands-check.py" $OUT/$tag.png --x $px --w 100 | sed 's/^/    /'
    sleep 1
}
...
vk_up() { setsid nohup python3 "$ROOT/tool/vkbd_linux.py" --fifo $FIFO >>$WORK/vkbd.log 2>&1 </dev/null & sleep 1.5; }
