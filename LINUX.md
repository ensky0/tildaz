...
- **Hyprland** — 실기 Hyprland 0.55.4 시연 완료 ([#227](https://github.com/ensky0/tildaz/issues/227), 2026-06): layer-shell drop-down + hidden_start + on_demand 클릭-어웨이 + install.sh 의 config bind→`--toggle` hotkey (portal 우회). 상세는 *Hyprland* 섹션. #3 (exit→재실행→F1) 재확인만 잔여.
...
**남은 검증 ([#3](https://github.com/ensky0/tildaz/issues/227) 후속 항목, 해소 추정).** `exit` 로 shell 종료 → surface 소멸 → wofi 등으로 tildaz 재실행 → F1 재동작이 정상인지 한 사이클 재확인. 안 되면 `~/.local/state/tildaz/tildaz.log` + `hyprctl binds | grep tildaz` 확인.
...
| **wlroots** | ✅ | native — sway = `bindsym` i3-ipc, Hyprland = config bind→`--toggle` (portal 우회) | sway [#226](https://github.com/ensky0/tildaz/issues/226) 🟨, Hyprland [#227](https://github.com/ensky0/tildaz/issues/227) 🟨 (실기 시연 OK, #3 재확인 잔여 — *Hyprland* 섹션) | Wayfire / river / niri |
