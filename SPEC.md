...
- **text-input-v3 IME (fcitx5) 가 떠 있을 때 누가 조합하는지는 입력기에 따라 갈린다** (#494 · [#530 실기](https://github.com/ensky0/tildaz/issues/530#issuecomment-5432460964)). `keyboard-us` 입력기는 fcitx5 가 자기 Compose 로 조합해 preedit · commit 을 보내므로 위 경계 규칙이 관측되지 않는다 (`^`+`x` 가 `^x`). `hangul` 입력기는 조합 중이던 한글을 확정한 뒤 **키를 앱으로 넘기므로** 앱의 Compose 와 조합 중 표시가 그대로 동작한다 — 한국어 사용자는 대개 이쪽이다. 한글 조합 중에는 IME 가 전부 소유한다 — "IME 가 주인" 규칙 그대로. 검증 결과에는 *조합 주체가 누구였는지* (앱 로그의 `text_input preedit/commit` 줄 수) 를 함께 적어야 한다.
...
조합 중 표시 (#530) 의 검증 (2026-08-27): lima headless sway (software 렌더) 에서 `grim` 캡처의 preedit 배경색 픽셀 151 → 0 과 스크린샷 확인, **COSMIC 1.6.0 실기 (GL 렌더 · 실제 `fr` 자판 · fcitx5 공존 포함) 7 케이스 전부 일치** ([#530 댓글](https://github.com/ensky0/tildaz/issues/530#issuecomment-5432460964)) — `^` 표시 · `e` 로 확정 · Enter / Ctrl+C / 단축키 / 포커스 이탈에서 지움 · `^´` 두 글자 덧붙임 · IME preedit 과 같은 프레임에 겹치지 않음. 두 렌더 경로가 모두 확인됐고 표시는 compositor 와 무관한 앱 내부 경로라 KDE 는 따로 재지 않았다.
