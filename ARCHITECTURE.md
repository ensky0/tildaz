...
| Session core | Yes | `src/session_core.zig` | Tabs — each a `TabGroup` of panes (`Tab`) arranged by a `pane_layout.Tree`, always one leaf until splitting lands ([#483](https://github.com/ensky0/tildaz/issues/483) step 3) — active index, scrollback, VT draining across all panes, PTY queues, and each pane's ghostty `RenderState` snapshot (panes outside the active tab release theirs) |
...
| Split panes | [#483](https://github.com/ensky0/tildaz/issues/483) | Slow track. Steps 1–3 of 6 landed: `src/pane_layout.zig` (pure split tree + per-pane grid, hit testing, neighbor search), the three renderers split into `drawPane(PaneDraw)` / `endFrame`, and `SessionCore` tabs became `TabGroup`s over that tree — all with zero visible change (pixel A/B identical on macOS and Linux; Windows pending). Next: actual splitting, key bindings, and focus display, Linux first |
