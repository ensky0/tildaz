...
|---|---|---|---|---|---|---|---|
| Drop-down 위치 | 다른 모든 윈도우 위 | `WS_EX_TOPMOST` + `SetWindowPos(HWND_TOPMOST)` | `NSPopUpMenuWindowLevel` (101) | `zwlr_layer_shell_v1.get_layer_surface(layer=top)` + `keyboard_interactivity=exclusive` (L8-α). layer-shell 미advertise compositor (mutter 등) 에선 xdg-shell fallback (위 보장 없음) | ✅ | ✅ | ✅ (layer-shell) / 🟨 (xdg-shell fallback) |
...
| Dock 위치 (config) | top / bottom / left / right | `setPosition` | `repositionWindow` | `set_anchor(top\|bottom\|left\|right)` ([5c203a0](https://github.com/ensky0/tildaz/commit/5c203a0), L8-β) | ✅ | ✅ | ✅ |
...
| Opacity (config) | 0..100 percent → alpha | `SetLayeredWindowAttributes` (LWA_ALPHA) | `NSWindow.setAlphaValue:` | ARGB8888 alpha sweep ([318bbef](https://github.com/ensky0/tildaz/commit/318bbef), L13-γ) | ✅ | ✅ | ✅ |
| Theme (config) | 16-color palette + bg/fg | `themes.findTheme` → ghostty Terminal.Colors | 동일 | 동일 (cross-platform `themes` 모듈) | ✅ | ✅ | ✅ |
| 단일 탭 시 탭바 | 자리 없음 (Cmd+Q/W 만 종료) | (탭바 항상 표시) | `tabBarHeightPx == 0` 분기 | ❌ — `tabBarHeightPx == 0` 분기 미구현, 단일 탭에서도 탭바 자리 차지 | ✅ | ✅ | ❌ |
...
| 동작 | Windows | macOS | Linux | Win | Mac | Linux |
|---|---|---|---|---|---|---|
| 복사 (단축키) | Ctrl+Shift+C | Cmd+C | Ctrl+Shift+C ([441f894](https://github.com/ensky0/tildaz/commit/441f894), L6) | ✅ | ✅ | ✅ |
...
| 붙여넣기 (단축키) | Ctrl+Shift+V | Cmd+V | Ctrl+Shift+V ([441f894](https://github.com/ensky0/tildaz/commit/441f894)) | ✅ | ✅ | ✅ |
| 붙여넣기 (마우스 우클릭) | `WM_RBUTTONDOWN` → `pasteClipboard` (cmd.exe console 표준 패턴) | 동일 (`tildazRightMouseDown` → `handlePaste`) | `wl_pointer.button` BTN_RIGHT → `wl_data_offer.receive` → PTY ([441f894](https://github.com/ensky0/tildaz/commit/441f894)) | ✅ | ✅ | ✅ |
...
| 동작 | Windows | macOS | Linux | Win | Mac | Linux |
|---|---|---|---|---|---|---|
| Ctrl+C → SIGINT (\\x03) | `WM_CHAR` 자동 ASCII control char | NSEvent.characters 직접 PTY write | xkb modifier check + utf8 fallback ([441f894](https://github.com/ensky0/tildaz/commit/441f894)) | ✅ | ✅ | ✅ |
| Ctrl+A ~ Ctrl+Z 일반 control | 동일 | 동일 (Ctrl+] tag jump, Ctrl+W vim window 등) | 동일 (xkb ctrl modifier compose) | ✅ | ✅ | ✅ |
| 한글 IME 조합 중 Ctrl+C | (해당 없음) | `discardMarkedText` + preedit overlay 비움 + \\x03 직송 — shell 의 "입력 라인 버리기" 의도와 일관 | text-input-v3 reset + preedit_buf 비움 + \\x03 ([6e46e49](https://github.com/ensky0/tildaz/commit/6e46e49), L10-γ) | — | ✅ | ✅ |
...
| 더블클릭 word selection | cell 영역 | `mouse_double_click` → `selectWord` | 동일 (`tildazMouseDown` clickCount >= 2) | 동일 ([dd40440](https://github.com/ensky0/tildaz/commit/dd40440), L6.7 — 500ms 같은 cell 검사 후 `selectWord`) | ✅ | ✅ | ✅ |
...
| 우클릭 paste | 어디든 | `WM_RBUTTONDOWN` → `pasteClipboard` | `tildazRightMouseDown` → `handlePaste` | `wl_pointer.button` BTN_RIGHT → `wl_data_offer.receive` ([441f894](https://github.com/ensky0/tildaz/commit/441f894)) | ✅ | ✅ | ✅ |
...
| 스크롤바 클릭 + 드래그 | 우측 가장자리 | `mouse.x >= client_w - SCROLLBAR_W` → `scrollToY` | 동일 (`scrollbarScrollToY`, Windows 패턴 그대로) | 동일 ([33b760b](https://github.com/ensky0/tildaz/commit/33b760b), L6.6 — Windows 패턴 그대로) | ✅ | ✅ | ✅ |
...
| 탭바 — × 클릭 | 탭 우측 close 박스 | `handleTabClick` → `closeTab` | 동일 | 동일 (L12-γ-4, [41473d9](https://github.com/ensky0/tildaz/commit/41473d9)) | ✅ | ✅ | ✅ |
| 탭바 — drag reorder | 탭 본체 drag | `DragState` (5px 임계) → `reorderTabs` | 동일 (`g_drag`) | 동일 (L12-γ-3, [b801baf](https://github.com/ensky0/tildaz/commit/b801baf)) | ✅ | ✅ | ✅ |
...
|---|---|---|---|---|---|---|---|
| 컬렉션 | `ArrayList(*Tab)` + `active_tab: usize` | `SessionCore` | `SessionCore` (v0.4.0+ 통합) | `SessionCore` 동일 (L12-α [b392765](https://github.com/ensky0/tildaz/commit/b392765)) | ✅ | ✅ | ✅ |
| 새 탭 크기 | 활성 탭의 cols/rows 와 동일 | `createTab(cols, rows, ...)` | 동일 | 동일 | ✅ | ✅ | ✅ |
| 단일↔멀티 전환 시 cell 영역 동기화 | 양쪽 모두: 단일 탭 시 탭바 자리 없음 → 전환 시 cell 영역 변화 → 모든 탭 cols/rows 재계산 (#127 으로 Windows 도 동등). | `effectiveTabBarHeight()` + `createTab` / `handleCloseResult` 의 `resizeAll` | `syncTerminalGeometry` 호출 | ❌ — §1 의 hide-tab-bar 분기 미구현이라 전환 시 cell 영역 변화 자체가 없음. 위 행 해결과 묶임 | ✅ | ✅ | ❌ |
...
|---|---|---|---|---|---|---|---|
| 조합 중 (preedit) inline 표시 | cursor 위치에 보라색 배경 + 글자 | `WM_IME_*` 가로채기 + `ImmGetCompositionStringW(GCS_COMPSTR)` → preedit_buf → cell / tab rename overlay (#164 v0.4.0) | `g_preedit_buf` + cell `renderFrame` 의 preedit 영역 + tab rename overlay | `zwp_text_input_v3.preedit_string` event → `preedit_buf` → cell / tab rename overlay ([6c685b6](https://github.com/ensky0/tildaz/commit/6c685b6), L10-β) | ✅ | ✅ | ✅ |
| 음절 단위 backspace | 자모 / 음절 단위 되돌리기 | (OS IME 자체) | 동일 | (fcitx5 / ibus 자체) | ✅ | ✅ | ✅ |
| 화살표 / 영문 / space → 음절 commit | IME 가 모르는 키 = 음절 자동 확정 | (OS IME 자체) | `interpretKeyEvents` → IME → callback | text-input-v3 의 commit_string + preedit_string done-apply batch ([6e46e49](https://github.com/ensky0/tildaz/commit/6e46e49), L10-γ) | ✅ | ✅ | ✅ |
...
| `hotkey` | string (`"f1"`, `"ctrl+space"`, `"shift+cmd+t"` 등). Windows 는 RegisterHotKey, macOS 는 CGEventTap. `cmd` 토큰 = Win key on Windows / Cmd / Super (Linux) on macOS. config = source of truth (cross-platform parity). | `f1` | `f1` | `f1` — `LinuxHotkey.fromString` 가 토큰 (`+` 분리) + xkb keysym (F1..F12 / a-z / 0-9 / space / grave / tab / escape / return) + modifier 비트마스크 (`MOD_ALT`/`MOD_CTRL`/`MOD_SHIFT`/`MOD_SUPER`) parse. `keysymToAccelerator` 가 **portal-kde 표준 형식** (`CTRL+grave`, `SHIFT+CTRL+a` — *대문자* modifier + `+` 분리. portal-kde 의 `XdgShortcut::parse` 가 case-sensitive HashMap 으로 *대문자만* 받음) 으로 portal `GlobalShortcuts.BindShortcuts` 송신. config ≠ system binding 시 **계층적 fallback chain** (`portal.handleHotkeyMismatch` — #207): 1차 XDG portal `UnbindShortcuts` (v2) → 2차 DE-specific (KDE: `org.kde.kglobalaccel.unregister` *binding 비움* + `setShortcut` *우리 keys 직접 set* — `unregister` 만으로는 entry 의 binding value 만 비우고 *entry 자체 유지* → portal-kde 가 entry 존재만 보고 empty 응답 반복하는 문제, 시연 진단으로 확정) → 3차 dialog 안내. 자동 update 후 `dialog.showInfo` overlay 로 사용자 OK 확인. | ✅ | ✅ | 🟨 (F1 ✅ / modifier 조합 + 자동 적용 ✅ 코드 구현, 사용자 시연 검증 대기 #207) |
