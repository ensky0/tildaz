...
# macOS 의 `tool/render-ab-shot/render-ab-shot_macos.sh` · Linux 의 `tool/render-ab-shot/render-ab-shot_linux.sh` 에 대응한다 — #586 Windows atlas `grow`
# 실기에서 만들었다. #584 의 Windows 계측 (같은 기기) 이 밟은 함정을 그대로 피한다.
#
# ```powershell
# tool\render-ab-shot\render-ab-shot_windows.ps1 -Screen <화면.cmd> -Size <격자> -Wait <대기초> -Tag <태그> -Bins <exeA>,<exeB>[,<exeC> …]
# tool\render-ab-shot\render-ab-shot_windows.ps1 -Screen $env:TEMP\stack2.cmd -Size 150x40 -Wait 8 -Tag stack2 -Bins $env:TEMP\tildaz-4096.exe,zig-out\bin\tildaz.exe
# ```
#
...
public static class TzShot {
  ...
  // ---- 합성 입력 (`-NewTab`) — `tool/send-keys_windows.ps1` 과 같은 규칙: SendInput · scan code 채움 ·
