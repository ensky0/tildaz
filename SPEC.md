...
| 단일 탭 시 탭바 | 자리 없음 (Cmd+Q/W 만 종료) | (탭바 항상 표시) | `tabBarHeightPx == 0` 분기 | (탭바 항상 표시) — mac 과 분기 차이 | ✅ | ✅ | 🟨 (mac 의 hide-tab-bar 분기 미적용) |
...
| 한글 자모 길게 누름 → 반복 입력 | (해당 없음) | IME 경로라 PressAndHold 영향 없음 (자동) | IME (fcitx5 / ibus) 경로 — 미검증 (자모 commit timing 관련) | — | ✅ | ❓ |
...
| 단일↔멀티 전환 시 cell 영역 동기화 | 양쪽 모두: 단일 탭 시 탭바 자리 없음 → 전환 시 cell 영역 변화 → 모든 탭 cols/rows 재계산 (#127 으로 Windows 도 동등). | `effectiveTabBarHeight()` + `createTab` / `handleCloseResult` 의 `resizeAll` | `syncTerminalGeometry` 호출 | (Linux 는 탭바 항상 표시 — §1 에 명시. 단일↔멀티 cell 영역 변화 없음) | ✅ | ✅ | 🟨 (mac 의 분기 미적용) |
...
| `hotkey` | string (`"f1"`, `"ctrl+space"`, `"shift+cmd+t"` 등). Windows 는 RegisterHotKey, macOS 는 CGEventTap. `cmd` 토큰 = Win key on Windows / Cmd on macOS. | `f1` | `f1` | `f1` — portal `GlobalShortcuts` 우선, 미가용 환경에선 `tildaz --toggle` Unix socket. `cmd` token Linux 동등 — 미정 (현 implementation 검증 필요) | ✅ | ✅ | 🟨 (F1 ✅ / 다른 modifier 조합 ❓) |
