...
//! `tool/osc-title-probe/osc-title-probe_linux.zig`. 출력 형식은 두 platform 결과를 같은 표로
...
//! ```powershell
//! zig build-exe tool/osc-title-probe/osc-title-probe_windows.zig -O ReleaseSafe -lc --cache-dir C:/ziglang/tildaz-cache
...
comptime {
    if (builtin.os.tag != .windows) {
        @compileError("osc-title-probe 는 Windows 전용 (ConPTY). Linux 는 tool/osc-title-probe/osc-title-probe_linux.zig 를 쓴다.");
    }
}
