...
- **`input` 은 셸 왕복을 포함해요.** 그래서 게이트와 `render` · `present` 를 빼고도 남는 몫이 있으면
  그건 **자식 프로세스 쪽**이에요 (우리 코드가 아니에요). 유휴 최악이 튄 회차에서 9.84 ms 중
  게이트 1.73 + render · present 1 ms 미만을 빼면 7 ms 가량이 그 자리였어요.
