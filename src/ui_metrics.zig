...
/// #646 — 검색 **현재 매치** (amber 채움) 위의 글자색.
///
/// `chrome_palette` 파생을 **타지 않는다** — `TAB_ACCENT_COLOR` 가 theme 과 무관한 고정
/// 밝은 색이라 그 위의 글자도 theme 과 무관하게 어두워야 한다. 밝은 theme 의 밝은 글자색
/// (`menu_label` 은 Catppuccin Latte 에서 `#060908` 이지만 배경 계열 색은 `#eff1f5`) 을
/// 쓰면 amber 위 대비가 2:1 아래로 떨어져 읽히지 않는다. 검정 대비는 10.4:1 이다.
pub const SEARCH_CURRENT_FG: [4]f32 = .{ 0.0, 0.0, 0.0, 1.0 };

/// #646 — **선택되지 않은** 검색 매치의 칸 배경 · 글자색. 어두운 amber 에 흰 글자다.
///
/// 처음에는 `chrome.menu_hover_bg` 를 썼는데 **화면에서 안 보였다** (2026-09-16 실기).
/// 당연한 결과였다 — chrome 팔레트의 색은 전부 *탭바 위에 얹히는 얌전한 표면색* 이고 한
/// 항목의 hover 를 알리도록 튜닝돼 있다. 게다가 회색이라 터미널 배경과 **색상이 같아서**,
/// 매치 수십 개가 셀 위에 흩뿌려지는 자리에서는 밝기 차이만으로 버텨야 했다.
///
/// **theme 에서 파생하지 않는다.** `TAB_ACCENT_COLOR` 와 같은 이유다. 파생을 시도해 보고
/// 접었는데, 재 보니 전제가 틀렸기 때문이다 — 밝은 theme 에서는 **현재 매치의 amber 조차
/// 배경 대비가 1.8:1** 이다. 그런데도 잘 보인다. 매치를 보이게 하는 것은 배경과의 밝기
/// 차이가 아니라 *채워진 색 블록 + 뒤집힌 글자색* 이기 때문이다. 그러면 theme 마다 색을
/// 바꿀 이유가 없고, 고정색이 18 종 어디서나 같은 값을 보장해 더 낫다.
///
/// 값은 세 조건을 동시에 만족하도록 골랐다 (`cell_color.zig` 의 테스트가 고정한다):
///   1. 흰 글자가 읽힌다 — 5.8:1 (WCAG AA 본문 4.5 위).
///   2. 18 종 theme 배경 모두에서 떨어져 보인다 — 최악 2.2:1 (가장 밝은 어두운 theme 인
///      Nord `#2e3440`). 사용자 기본 theme 인 Tilda (순수 검정) 에서는 3.7:1 이다.
///   3. 현재 매치와 구별된다 — 둘 사이 2.8:1 에 글자색이 검정↔흰색으로 뒤집힌다.
///
/// **한 번 더 낮춘 값이다** (2026-09-16 실기). 처음 고른 `#9b660f` 는 매치가 수십 개 깔릴
/// 때 화면이 얼룩졌다. 낮추면 ①③ 은 좋아지고 ② 만 나빠지는 한 방향 교환이라, ② 가 버티는
/// 선까지 내렸다.
///
/// **더 내리면 안 된다.** Nord 에서 2:1 아래로 떨어지는데, 그것이 바로 옛 `menu_hover_bg`
/// 가 검정에서 갖던 값 (2.03) 이고 "칠했는데 안 보인다" 가 됐던 자리다. 지금 Nord 에서
/// 2.2 로 버티는 것은 밝기가 아니라 **색상 차이** 덕이다 — amber 대 청회색이라 색상환에서
/// 가장 먼 쪽이고, WCAG 명도 대비는 그 차이를 세지 않는다.
pub const SEARCH_MATCH_BG: [4]f32 = .{ 140.0 / 255.0, 92.0 / 255.0, 14.0 / 255.0, 1.0 };
pub const SEARCH_MATCH_FG: [4]f32 = .{ 1.0, 1.0, 1.0, 1.0 };
