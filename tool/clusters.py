...
만들어 atlas 에 담기는 **종 수**가 예측 가능하다 (아래 표는 기본 인자 기준 · 실측은 #584 · #585 · #591).
    ...
    python3 tool/clusters.py many     > /tmp/many.sh      # 2,938 종 · 88x33 · 일반 화면 (grow 0 이어야 한다)
    python3 tool/clusters.py stack2   > /tmp/stack2.sh    # 6,000 종 · 150x40 · mark 둘 — 1024² 가 찬다 (안전망 검증)
    python3 tool/clusters.py overflow > /tmp/overflow.sh  # 17 화면 · 96,768 종 누적 · 150x40 — 8192² 상한에 닿는다
    python3 tool/clusters.py mini     > /tmp/mini.sh      # 1.5 s 뒤 한 줄 · 4 s 뒤 종료 — 렌더 프레임 수 계측용
    chmod +x /tmp/*.sh
...
def bands():
    ...
    `tool/bands-check.py` 가 그 단면을 읽는다. 터미널 크기를 스크립트 안에서 `stty size` 로 읽으므로
