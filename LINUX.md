...
| portal hotkey 와 config 자동 적용 (#207, 이전 #198 의 L9-δ Option Y 갱신) | 🟨 코드 구현 완료, 사용자 시연 검증 대기. **정책 변경 (#198 → #207)**: 이전 결정 (Option Y, *notification 만 + 사용자 수동 config 수정*) 의 *전제* = *우리 own dialog backend 없었음*. #203 Phase C step 3/4 로 layer-shell overlay dialog 가용 → 정책 재검토. 새 정책 — **config = source of truth + system binding → config 값으로 자동 update + 우리 dialog 안내** (mac/win 의 *config 우선* 정책 동등). 구현: 시작 시 portal `BindShortcuts` response 의 `actual_trigger` 와 우리 `preferred_trigger` 다르면 (empty actual 도 mismatch 처리) `portal.handleHotkeyMismatch` 의 **계층적 fallback chain**: <br/>① 1차 (cross-DE 정공): XDG portal `UnbindShortcuts` (spec v2 since) + 재 `BindShortcuts`. <br/>② 2차 (DE-specific): 1차 실패 시 `XDG_CURRENT_DESKTOP` 감지 + DE native API. **KDE**: ⓐ `org.kde.kglobalaccel.unregister(component, shortcut)` D-Bus call — runtime cache 의 binding 비움. ⓑ `org.kde.kglobalaccel.setShortcut(actionId, keys, NoAutoloading)` — KGlobalAccel store 의 `[tildaz] toggle` binding 을 우리 keys 로 *직접 set*. xkb keysym + modifier → Qt KeySequence packed int (`xkbToQtKey` 변환). ⓒ 재 `BindShortcuts` — portal-kde 의 internal cache 갱신 + actual 응답 확인. **GNOME / sway / hyprland**: 별 후속 작업 (#207 미구현). <br/>③ 3차 fallback: 모든 path 실패 / 미지원 DE → `dialog.showInfo` (layer-shell overlay) 로 *수동 변경 안내*. <br/>**사용자 시연 발견 (#207 진행 중) — 시도/실패 chain**: <br/>(1) KDE Plasma 6.6.5 의 portal-kde 가 `UnbindShortcuts` 미구현 (`"No such method"` D-Bus error). portal-kde upstream 의 v2 추가 시 1차 cross-DE 정공으로 동작 예정. <br/>(2) portal-kde 의 `XdgShortcut::parse` (`src/xdgshortcut.cpp`) 가 *대문자 modifier만* (`CTRL`/`SHIFT`/`ALT`/`LOGO`) case-sensitive HashMap — 소문자 `ctrl+grave` 보내면 *parse 실패* → user dialog 의 binding field "없음" 으로 표시. `keysymToAccelerator` 를 대문자 format 으로 수정. <br/>(3) `KGlobalAccel.unregister` (소문자) 는 *binding value 만 비움* (`toggle=,none,...`), entry 자체 유지. portal-kde 의 BindShortcuts 가 *entry 존재만 봄* → empty trigger_description 응답. `Component.cleanUp()` 은 *isActive=true 거부* (우리 process running). → `setShortcut` 으로 *우리 keys 직접 set* 가 정공. |
