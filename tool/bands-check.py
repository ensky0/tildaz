...
`tool/clusters.py bands` 화면 (밝은 230 / 어두운 20 띠가 3 줄씩) 을 띄운 창을 캡처해 넘긴다.
창 안 x 구간의 행별 평균 밝기를 내고, 두 띠 값 사이에 있는 행 (= 전이 행) 의 밝기를 모은다.

    python3 tool/bands-check.py shot.png            # 캡처 전체가 창일 때
    python3 tool/bands-check.py shot.png 2136x1261+744+0   # 창 영역 (ImageMagick geometry)
    python3 tool/bands-check.py shot.png --x 400 --w 100     # 단면 x 구간 (기본 창 폭의 중앙 100 px)
