...
if [ ! -x "$CAP" ] || [ "$REPO/tool/color-capture_macos.m" -nt "$CAP" ]; then
  mkdir -p "$(dirname "$CAP")"
  clang -fobjc-arc -framework Cocoa -framework ScreenCaptureKit -framework ImageIO \
        -framework UniformTypeIdentifiers -o "$CAP" "$REPO/tool/color-capture_macos.m" || exit 1
fi
...
for FMT in "$@"; do
  ...
  printf '#!/usr/bin/env bash\nexec bash %s/tool/mouse-probe/mouse-probe.sh %s %s --log %s\n' "$REPO" "$TRACKING" "$FMT" "$LOG" > "$WRAP"; chmod +x "$WRAP"
