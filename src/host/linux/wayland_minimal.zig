...
const gl_rects = @import("../../renderer/linux/gl_rects.zig");
const gl_atlas = @import("../../renderer/linux/gl_atlas.zig");
const gl_text = @import("../../renderer/linux/gl_text.zig");
...
const wp_cursor_shape_v1_row_resize: u32 = 31;
...
const Client = struct {
    ...
    fn pointerInCellArea(self: *const Client) bool {
        ...
        // #483 4b — 포인터 아래 pane 의 셀 영역 (padding · scrollbar 자리 안쪽) 인가. 분할선 · 탭바 ·
        // 영역 밖은 pane 이 없어 false. pane 하나면 이전의 창 기준 판정과 같다.
        var buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.PaneRect = undefined;
        const lay = self.activeLayout(&buf);
        const id = pane_layout.paneAt(lay, x, y) orelse return false;
        const pr = pane_layout.find(lay, id) orelse return false;
        const m = self.paneMetrics();
        if (x < pr.grid_x or y < pr.grid_y) return false; // 좌측 / 상단 padding
        if (y >= pr.rect.y + pr.rect.h - m.pad) return false; // 하단 padding
        if (x >= pr.rect.x + pr.rect.w - m.pad - m.scrollbar_w) return false; // 우측 padding · 스크롤바
        return true;
    }
    ...
    /// #483 4b — pane 격자 계산 입력. `gridSize` 가 쓰는 값에 분할선 두께를 더한 것.
    fn paneMetrics(self: *const Client) pane_layout.Metrics {
        return .{
            .cell_w = self.renderer.cellWidth(),
            .cell_h = self.renderer.cellHeight(),
            .pad = self.renderer.paddingPx(),
            .scrollbar_w = self.renderer.scrollbarWPx(),
            .separator_w = @intFromFloat(ui_metrics.linePx(ui_metrics.PANE_SEPARATOR_W_PT, self.renderer.scale)),
        };
    }

    /// #483 4b — 탭바를 뺀 터미널 영역 (`pane_layout.layout` 의 `rect`). 창 크기 기준이다 — 프레임
    /// 조립 (`frameInputs`) 은 buffer 크기로 같은 것을 따로 만든다.
    fn paneArea(self: *const Client) pane_layout.Rect {
        const tab_bar_h = self.effectiveTabBarHeightPx();
        return .{ .x = 0, .y = tab_bar_h, .w = self.window_width, .h = self.window_height - tab_bar_h };
    }

    /// #483 4b — 활성 탭의 pane 배치. 결과는 `buf` 안을 가리킨다. 세션 · 탭이 없으면 빈 slice.
    fn activeLayout(self: *const Client, buf: *[pane_layout.MAX_PANES_PER_TAB]pane_layout.PaneRect) []pane_layout.PaneRect {
        const sess = if (self.session) |*s| s else return buf[0..0];
    ...
    /// #483 4c — 활성 탭의 회색 분할선 목록 (최대화 중이면 없다). 결과는 `buf` 안을 가리킨다.
    fn activeSeparators(self: *const Client, buf: *[pane_layout.MAX_PANES_PER_TAB]pane_layout.Separator) []pane_layout.Separator {
        const sess = if (self.session) |*s| s else return buf[0..0];
        if (sess.active_tab >= sess.tabs.items.len) return buf[0..0];
        return sess.tabs.items[sess.active_tab].separators(self.paneArea(), self.paneMetrics(), buf);
    }

    /// #483 4c — 포인터 아래의 분할선 (양쪽 `PANE_SEPARATOR_HIT_SLOP_PT` 까지). dialog · 메뉴가 떠 있으면 null.
    fn separatorUnderPointer(self: *const Client) ?pane_layout.Separator {
        if (self.dialog.active() or self.command_menu_open) return null;
        var buf: [pane_layout.MAX_PANES_PER_TAB]pane_layout.Separator = undefined;
        const slop: i32 = ui_metrics.scaledPx(i32, ui_metrics.PANE_SEPARATOR_HIT_SLOP_PT, self.renderer.scale);
        return pane_layout.separatorAt(self.activeSeparators(&buf), self.pointer_x_px, self.pointer_y_px, slop);
    }

    /// #483 4c — 커서 모양의 근거. 드래그 중이면 그 축의 리사이즈 커서를 유지한다 (포인터가 선을 벗어나도).
    fn pointerRegion(self: *const Client) PointerRegion {
        ...
        // #646 — 검색바는 셀 위에 떠 있다. 입력칸은 I-beam 그대로 두고 (셀 영역이 이미
        // 그렇다) 컨트롤 · 여백은 화살표로 돌린다.
        if (self.searchBarViewConst()) |v| {
            const sc = self.renderer.scale;
            const x = @as(f32, @floatFromInt(self.pointer_x_px)) / sc;
            const y = @as(f32, @floatFromInt(self.pointer_y_px)) / sc;
            if (search_bar.contains(v, x, y)) {
                const in_field = x >= v.field.x and x < v.field.x + v.field.w;
    ...
    fn activePaneRect(self: *const Client) ?pane_layout.PaneRect {
    ...
    /// #483 4b — 활성 pane 의 첫 셀 좌상 px. 세션이 없으면 pane 하나일 때의 값 (`pad` · `tab_bar_h + pad`).
    fn activeGridOrigin(self: *const Client) struct { x: i32, y: i32 } {
        if (self.activePaneRect()) |pr| return .{ .x = pr.grid_x, .y = pr.grid_y };
        const pad = self.renderer.paddingPx();
        return .{ .x = pad, .y = self.effectiveTabBarHeightPx() + pad };
    }

    /// #483 4b — pane 의 scrollbar track 을 윗변에서 내리는 양. 단일 탭의 컨트롤 스트립 (`[+][×][…]`,
    /// #329) 은 오른쪽 위에 얹히므로 **그 모서리를 가진 pane 만** 내린다 (pane 하나면 이전의
    /// `scrollbar_top − tab_bar_h` 와 같은 값). 탭바가 있으면 (탭 2개 이상) 스트립이 탭바 안이라 0.
    fn paneScrollbarTopInset(self: *const Client, rect: pane_layout.Rect, area: pane_layout.Rect) i32 {
        const inset = self.scrollbarTopInsetPx() - self.effectiveTabBarHeightPx();
        if (inset <= 0) return 0;
        const top_right = rect.y == area.y and rect.x + rect.w == area.x + area.w;
        return if (top_right) inset else 0;
    }

    /// #483 4b — 포인터가 활성 pane 의 scrollbar 자리 (오른쪽 `scrollbar_w` 폭, pane 높이 전체) 안인가.
    /// pane 하나면 이전의 `x >= window_width − scrollbar_w` 와 같다.
    fn pointerInActiveScrollbarColumn(self: *const Client) bool {
        const pr = self.activePaneRect() orelse return false;
        const sbw = self.renderer.scrollbarWPx();
        const x = self.pointer_x_px;
        const y = self.pointer_y_px;
        return x >= pr.rect.x + pr.rect.w - sbw and x < pr.rect.x + pr.rect.w and y >= pr.rect.y and y < pr.rect.y + pr.rect.h;
    }
    ...
    fn frameInputs(
        ...
        // #483 4b — 활성 탭의 pane 마다 `PaneDraw` 하나 (`pane_layout.layout` 순서). rect 는 탭바를 뺀
        // 영역을 트리로 나눈 것 — pane 하나면 `{0, tab_bar_h, W, H − tab_bar_h}` 라 2단계와 같은 값이다.
        // 영역은 이 프레임의 buffer 크기로 잡는다 (격자는 `ensureSessionGrid` 가 창 크기로 잡는데,
        // 둘이 다른 순간은 configure 직후 한 프레임뿐이다).
        const tab_bar_h = self.renderer.tabBarHeightPx(count);
        const area: pane_layout.Rect = .{ .x = 0, .y = tab_bar_h, .w = width, .h = height - tab_bar_h };
        const m = self.paneMetrics();
        ...
        const seps = group.separators(area, m, sep_storage);
        // 4c — 드래그 중이면 트리 복사본에 놓아 보고 그 분할선 자리를 고스트로 (스냅 · 최소 크기 판정을
        // 실제 놓기와 같은 함수가 한다). 놓을 수 없는 자리면 고스트 없음.
        ...
        // 값이라 마우스 경로에서 다시 구하면 어긋날 여지가 생긴다. 실제 대입은 아래 루프가
        // `active_grid_bottom_px` 를 채운 뒤에 한다.
        for (lay, 0..) |pr, i| {
            ...
            pane_storage[i] = .{
                ...
                .cell_h = m.cell_h,
                .pad = m.pad,
                .scrollbar_w = @floatFromInt(m.scrollbar_w),
        ...
        // 스크롤바 판정은 그리는 쪽과 같은 기준이다 — 스크롤백이 보이는 영역보다 길면 뜬다
        // (`scrollbar.geom`).
        {
            const t = group.activeTab();
        ...
        return .{
            ...
            .blink_faint = self.last_blink_phase,
            // #646 — 검색바는 활성 pane 의 상태를 비춘다. 배치는 바로 위에서 잰 값이다.
            .search_ui = search_bar.uiFrom(
                &group.activeTab().search,
                if (search_focused_now) self.preedit_text.items else &.{},
                search_focused_now,
                self.search_hover,
    ...
    fn searchFocused(self: *Client) bool {
        const session = if (self.session) |*s| s else return false;
        const tab = session.activeTab() orelse return false;
    ...
    fn searchApplyKey(self: *Client, k: search_input.Key) void {
    ...
    /// #646 — 확정된 글자를 검색어에 끼운다. 타이핑 · dead key 조합 · IME 확정 · paste 가
    /// 모두 이리로 온다.
    fn searchInsertText(self: *Client, text: []const u8) void {
        const session = if (self.session) |*s| s else return;
        const tab = session.activeTab() orelse return;
    ...
    /// 경로로 넘기지 않는다. 바가 터미널 셀 **위에 떠 있어서** 가로채지 않으면 바 위 드래그가
    /// 터미널 선택을 만들고 마우스 리포팅을 켠 앱에 좌표가 전송된다.
    fn searchBarMouseDown(self: *Client) bool {
        const v = self.searchBarViewNow() orelse return false;
        const s = self.renderer.scale;
        const x = @as(f32, @floatFromInt(self.pointer_x_px)) / s;
        const y = @as(f32, @floatFromInt(self.pointer_y_px)) / s;
        if (!search_bar.contains(v, x, y)) return false;

        const session = if (self.session) |*sess| sess else return true;
        const tab = session.activeTab() orelse return true;
        ...
        if (x >= v.field.x and x < v.field.x + v.field.w) {
            const cw = @as(f32, @floatFromInt(self.renderer.tab_font_ctx.cell_width_px)) / s;
    ...
    /// #646 — xkb keysym → 검색 입력칸이 뜻을 두는 키. `null` 이면 입력칸이 안 쓰는 키다.
    fn searchKeyFromSym(sym: u32, shift: bool) ?search_input.Key {
        return switch (sym) {
            // Return / KP_Enter — 기본은 아래로, Shift 면 위로.
            xkb_key_return, 0xff8d => if (shift) .match_up else .match_down,
            xkb_key_escape => .close,
    ...
    fn searchNavSym(sym: u32) bool {
        return switch (sym) {
            0xff52, 0xff54, xkb_key_page_up, xkb_key_page_down, 0xff63, 0xff09 => true,
            else => false, // Up · Down · Page_Up · Page_Down · Insert · Tab
        };
    }
    ...
    fn settlePendingInput(self: *Client, mode: enum { commit, abandon }) void {
        ...
        if (self.preedit_text.items.len > 0) {
            // #646 — commit 은 **지금의 sink** 로 간다. 검색바가 키보드를 갖고 있으면
            // 검색어, 아니면 PTY 다 (`input_policy.Pending.commit` 이 뜻하는 것).
            if (mode == .commit) {
                if (self.searchFocused())
                    self.searchInsertText(self.preedit_text.items)
                else
    ...
    /// #483 4b — 포인터 아래 pane 이 활성 pane 이 아니면 그 pane 으로 포커스를 옮기고 true. 분할선 위 ·
    /// 탭바 · pane 하나 · 이미 활성이면 false. 떠나는 pane 에 preedit 을 확정하고 진행 중 pointer mode 를
    /// 정리한 **뒤** 옮긴다 — 순서가 바뀌면 자모가 새 pane 의 PTY 로 간다.
    fn focusPaneUnderPointer(self: *Client) bool {
        const session = if (self.session) |*s| s else return false;
    ...
    fn computeCursorRect(self: *const Client) struct { x: i32, y: i32, w: i32, h: i32 } {
        const cw = self.renderer.cellWidth();
        const ch = self.renderer.cellHeight();
        // #483 4b — 활성 pane 의 격자 원점 (pane 하나면 `pad` · `tab_bar_h + pad`).
        const origin = self.activeGridOrigin();
        if (if (self.activeRenderState()) |st| st.cursor.viewport else null) |vp| {
            const x: i32 = origin.x + @as(i32, @intCast(vp.x)) * cw;
            const y: i32 = origin.y + @as(i32, @intCast(vp.y)) * ch;
            return .{ .x = x, .y = y, .w = cw, .h = ch };
        }
        return .{ .x = origin.x, .y = origin.y, .w = cw, .h = ch };
    }
    ...
    fn applyTextInputBatch(self: *Client) !void {
        if (self.pending_commit.items.len > 0) {
            ...
            // #646 — 검색 중이면 확정 글자는 PTY 가 아니라 검색어로 간다.
            if (self.searchFocused())
                self.searchInsertText(self.pending_commit.items)
            else
        ...
        self.pending_preedit.clearRetainingCapacity();
        // #646 — 검색바가 키보드를 가지면 조합 글자는 터미널 커서가 아니라 **입력칸 안**에
        // 보인다. 조합이 보이는 자리는 하나여야 한다.
        self.renderer.preedit_text = if (self.searchFocused()) "" else self.preedit_text.items;
        ...
        if (self.preedit_text.items.len > 0 and !self.searchFocused()) {
    ...
    fn processKeyEvent(self: *Client, serial: u32, key: u32, action: key_encode.Action) !void {
        ...
        if (sym_opt) |sym| {
            ...
            if (classifyInput(bindings, fallbacks, sym, unshifted, code, ctrl, shift, alt, super)) |classified| {
                ...
                const disp = input_policy.resolve(classified.input, .{
                    // #646 — 조합 중 글자가 검색 입력칸으로 갈 때는 *터미널* preedit 이
                    // 아니다. sink 가 다르므로 `search_active` 쪽으로 센다.
                ...
                switch (disp.target) {
                    ...
                    .pty => {},
                    // #646 — 검색 중 Ctrl+C 는 삼킨다. 검색어를 치다가 셸이 SIGINT 를 받으면
                    // 안 되고, 벗어나는 키는 Esc 다. (`.search_field` 는 `.paste` · `.text` ·
                    // `.edit_key` 의 target 인데 셋 다 이 분기 앞에서 처리된다.)
        ...
        //
        // 자리가 여기인 이유가 둘이다. 위의 `[keys]` 조회보다 **뒤**라 검색 중에도 단축키가
        // 그대로 돈다 (검색바 포커스 중 pane 이동). 아래 compose · PTY 경로보다 **앞**이라
        // 편집키가 셸로 새지 않는다.
        if (self.searchFocused()) {
            if (sym_opt) |sym| {
                if (searchKeyFromSym(sym, shift)) |k| {
                    if (input_policy.resolve(.edit_key, .{ .search_active = true }).target == .search_field) {
                        self.searchApplyKey(k);
                        return;
                    }
                } else if (ctrl and !alt) {
                    // Ctrl 조합은 한 줄 입력칸이 쓰지 않는다. Ctrl+C 는 `interrupt`, 나머지는
                    // `nav_key` 인데 검색 중에는 **둘 다 `drop`** 이라 결과가 같다.
                    if (input_policy.resolve(.interrupt, .{ .search_active = true }).target == .drop) return;
                } else if (searchNavSym(sym)) {
                    if (input_policy.resolve(.nav_key, .{ .search_active = true }).target == .drop) return;
                }
            }
            // 그 밖의 키는 글자다 — 아래 compose · utf8 경로가 만든 텍스트를 그 자리에서
            // 입력칸으로 보낸다 (dead key 조합이 검색어에서도 그대로 된다).
        }
        ...
        if (sym_opt) |sym| {
            ...
            if (shift and !ctrl and !alt and (sym == xkb_key_page_up or sym == xkb_key_page_down)) {
                self.commitPendingInput();
                if (self.session) |*session| {
                    // #483 4b — 한 페이지 = 활성 pane 의 행 수 (pane 마다 다르다). pane 하나일 때 창
                    // 높이에서 계산한 이전 값과 같다.
        ...
        if (sym_opt) |sym| {
            if (self.preedit_text.items.len == 0 and !ctrl and !alt) {
                ...
                switch (self.keyboard.composeFeed(sym, &buf)) {
                    ...
                    .composed => |text| {
                        self.dropComposePreview();
                        // #646 — 검색 중이면 조합 결과도 입력칸으로. dead key 가 검색어에서
                        // 그대로 동작한다 (`^` + `e` → `ê`).
                        if (self.searchFocused()) self.searchInsertText(text) else self.queueInput(text);
                        return;
                    },
                }
            }
        ...
        const utf8_out = self.keyboard.utf8(xkb_key, &buf);
        // #646 — 검색 중에는 **글자만** 입력칸으로 보낸다. 제어문자 · 특수키는 위 분기에서
        // 이미 걸렀고, 여기 남은 제어문자는 입력칸이 쓸 것이 없으므로 삼킨다.
        if (self.searchFocused()) {
            if (utf8_out.len > 0 and utf8_out[0] >= 0x20 and utf8_out[0] != 0x7f) {
                self.searchInsertText(utf8_out);
            }
            return;
        }
        self.sendEncodedKey(key, xkb_key, utf8_out, action);
    }
    ...
    fn handlePointerMotion(self: *Client, payload: []const u8) void {
        ...
        if (self.search_press) return;
        // 검색바 컨트롤 hover. 바 위에 있으면 아래 탭바 · 터미널 판정을 건너뛴다 (바가 위다).
        {
            const v = self.searchBarViewNow();
            const sc = self.renderer.scale;
            const hx = @as(f32, @floatFromInt(self.pointer_x_px)) / sc;
            const hy = @as(f32, @floatFromInt(self.pointer_y_px)) / sc;
            ...
            if (v) |vv| if (search_bar.contains(vv, hx, hy)) return;
        }
        ...
        // #483 4c — 분할선 드래그 중: 좌표만 기억하고 고스트를 다시 그린다. 트리는 놓을 때.
        if (self.sep_drag) |*d| {
            d.px = if (d.axis == .side_by_side) self.pointer_x_px else self.pointer_y_px;
            self.needs_redraw = true;
            return;
        }
    ...
    fn handlePointerButton(self: *Client, payload: []const u8) void {
        ...
        if (button == wl_pointer_button_right) {
            ...
            // #483 4b — 비활성 pane 우클릭은 포커스만 옮기고 붙여넣지 않는다 (확정 설계 축 3).
        ...
        // #483 4b — 누름이 다른 pane 으로 포커스를 옮기면 아래에서 다시 잡는다.
        var tab = self.activeTabOrNull() orelse return;
        switch (state) {
            wl_pointer_button_state_pressed => {
                ...
                // #483 4c — 분할선 (±slop) 누름 = 드래그 시작. pane 안 판정으로 내려가지 않는다.
                if (self.separatorUnderPointer()) |s| {
                    self.commitPendingInput();
                    self.sep_drag = .{ .node = s.node, .axis = s.axis, .px = if (s.axis == .side_by_side) self.pointer_x_px else self.pointer_y_px };
                    self.needs_redraw = true;
                    return;
                }
                // #483 4b — 다른 pane 을 눌렀으면 먼저 그 pane 으로 포커스를 옮기고, 그 pane 기준으로
                // 아래 scrollbar · 셀 판정을 이어 간다 (포커스 이동과 선택 시작이 한 클릭).
                if (self.focusPaneUnderPointer()) tab = self.activeTabOrNull() orelse return;
                // 활성 pane 의 스크롤바 자리 클릭 — selection / 더블클릭 보다 우선.
                // Windows `app_controller.zig:835` 와 동등.
                if (self.pointerInActiveScrollbarColumn()) {
    ...
    fn scrollbarHit(self: *Client) ?scrollbar.Hit {
        ...
        // #483 4b — track 은 활성 pane 기준. 렌더러 `collectScrollbar` 의 `thumbRect` 인자와 같은 값 —
        // pane 의 아래 가장자리와 `rect.y + inset` (pane 하나면 이전의 `window_height` · `scrollbarTopInsetPx`).
    ...
    fn pasteFromClipboard(self: *Client) void {
        ...
        if (self.session != null) self.routePasteToSink(accumulated.items);
    }

    /// #646 — paste payload 를 **지금의 sink** 로. 검색바가 키보드를 갖고 있으면 검색어,
    /// 아니면 PTY 다 (`tab_actions.routePaste` — bracketed paste · wrap 은 그쪽이 든다).
    fn routePasteToSink(self: *Client, text: []const u8) void {
        if (self.searchFocused()) {
            self.searchInsertText(text);
            return;
        }
        var host = self.buildTabActionsHost();
    ...
    fn pixelToCell(self: *const Client, px: i32, py: i32) ?terminal_interaction.Cell {
        ...
        const pr = self.activePaneRect() orelse return null;
        const c = pane_layout.cellAt(pr, px, py, self.paneMetrics()) orelse return null;
    ...
    fn selectionCellAndDir(self: *Client, tab: *session_core.Tab) struct { cell: terminal_interaction.Cell, dir: i8 } {
        // #483 4b — 활성 pane 의 격자 원점 기준.
        const origin = self.activeGridOrigin();
        const cw = self.renderer.cellWidth();
        const ch = self.renderer.cellHeight();
        const cols: u16 = tab.terminal.cols;
        const rows: u16 = tab.terminal.rows;
        const col_i32: i32 = @divFloor(self.pointer_x_px - origin.x, cw);
        const row_i32: i32 = @divFloor(self.pointer_y_px - origin.y, ch);
        return .{
...
const SepDrag = struct {
    node: u8,
    axis: pane_layout.Axis,
    /// 분할 축의 포인터 좌표 (세로 분할선이면 x, 가로면 y).
    px: i32,
};

fn cursorShapeForSurface(focused_surface_id: u32, main_surface_id: u32, region: PointerRegion) u32 {
...
test "#314 dialog surface keeps arrow cursor while main text areas use I-beam" {
    try std.testing.expectEqual(
        ...
        cursorShapeForSurface(200, 100, .cell),
    );
    try std.testing.expectEqual(
        wp_cursor_shape_v1_text,
        cursorShapeForSurface(100, 100, .cell),
    );
    try std.testing.expectEqual(
        wp_cursor_shape_v1_default,
        cursorShapeForSurface(100, 100, .other),
    );
    try std.testing.expectEqual(
        wp_cursor_shape_v1_default,
        cursorShapeForSurface(0, 0, .cell),
    );
    // #483 4c — 분할선 위는 리사이즈 커서, 다른 surface 위에서는 여전히 기본.
    try std.testing.expectEqual(wp_cursor_shape_v1_col_resize, cursorShapeForSurface(100, 100, .separator_v));
    try std.testing.expectEqual(wp_cursor_shape_v1_row_resize, cursorShapeForSurface(100, 100, .separator_h));
    try std.testing.expectEqual(wp_cursor_shape_v1_default, cursorShapeForSurface(200, 100, .separator_v));
}
