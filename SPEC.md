...
| Rename — IME 후보 popup 위치 | cursor 옆 자연 추적 (한자 / kanji / hanzi) | `ImmSetCompositionWindow(CFS_POINT, cursor_pixel)` 매 frame (#164 v0.4.0) | OS IME 가 자체 anchor (NSTextInputContext) | ✅ | ✅ |
...
| 한자 / kanji / hanzi 후보 popup 위치 | cursor 옆 추적 | `ImmSetCompositionWindow(CFS_POINT, cursor_pixel)` 매 frame (#164 v0.4.0) | OS IME 자체 — NSTextInputContext 의 firstRectForCharacterRange (cell 영역만, tab rename 미구현) | ✅ | 🟨 |
| Hanja 변환 (Option+Return) | committed text → 한자 후보 popup | (해당 없음 — Win 은 한자 키 즉시 popup) | NSTextInputClient reconversion API (`attributedSubstringForProposedRange` 등) 미구현 — Apple Terminal.app 도 동일, `NSTextView` 만 작동. #166 별 enhancement issue. | — | 🟨 #166 |
...
**구현 노트:**
- mac: `tildazKeyDown` rename 분기에 직접 keyCode intercept (115/119 = Home/End, Ctrl+0/14 = Ctrl+A/E). `interpretKeyEvents` → Cocoa StandardKeyBinding 의존 X — 우리 custom NSView 에선 dispatch 안 되는 selector 가 있어 신뢰성 낮음.
- mac: `commitPreeditPreserving` helper — preedit 만 commit (rename 유지). Cmd 단축키 / mouse click 의 `commitPendingInput` 도 이 helper + `commitOrCancelRename(true)` 로 분리.
- win: `WM_KEYDOWN` 에서 Ctrl+A/E (no Shift) → `KeyInput.home/.end` dispatch. rename 비활성 시 fall-through (셸 readline 정상 0x01/0x05 PTY 송신).
- 양 platform: `tab_layout.iterTabText` 가 cursor 따라 viewport scroll (RenameState.scroll_offset cached state, #168) — nav 후 cursor 위치 자동 따라옴.
- deferred: rename 중 긴 텍스트의 hidden 영역 visual cue ("..." 등) — #169.
...
| `window.width` | 1..100 percent | 50 | 50 | ✅ | ✅ |
| `window.height` | 1..100 percent | 100 | 100 | ✅ | ✅ |
| `window.offset` | 0..100 percent | 100 | 100 | ✅ | ✅ |
| `window.opacity` | 0..100 percent | 100 | 100 | ✅ | ✅ |
| `theme` | string (`themes.findTheme`) | `Tilda` | `Tilda` | ✅ | ✅ |
| `font.family` | string / array (max 8) | `["Cascadia Code"]` (single — system fallback 이 한글 / CJK / 기호 / 이모지 자동 처리) | `["Menlo"]` (single — system fallback 동일) | ✅ | ✅ |
| `font.size` | integer 8..72 | 16 | 15 (Apple Terminal/iTerm2 컨벤션 + retina) | ✅ | ✅ |
| `font.line_height` | float | 1.0 (#150 — DWrite native) | 1.1 (Apple HIG) | ✅ | ✅ |
| `font.cell_width` | float | 1.0 (#150 — DWrite native) | 1.0 (Menlo metric 자연) | ✅ | ✅ |
| `shell` | string (셸 경로) | `cmd.exe` | config.shell 명시값 > `$SHELL` env > `/bin/zsh`. 빈 문자열 = default fallback. | ✅ | ✅ |
...
> **font.family glyph fallback chain** (#135): codepoint 별로 chain 순회 → 글리프 가진 첫 폰트 사용. chain 에 없는 codepoint 는 양쪽 OS 모두 system fallback 이 자동 처리 — Windows DirectWrite `IDWriteFontFallback.MapCharacters` (Hangul → Malgun Gothic / Symbols → Segoe UI Symbol / Emoji → Segoe UI Emoji 등 OS 내장 chain 표), macOS CoreText `CTFontCreateForString`. 사용자가 다른 한글 폰트 등을 명시하고 싶으면 array 로 override (예: `["Menlo", "Apple SD Gothic Neo"]`).
>
> 모든 명시 폰트가 system 에 register 되어 있어야 함 — 하나라도 없으면 fatal dialog (`font_validate.showNotFoundFatal`, chain dump + 미설치 표시 + config 경로). macOS substitute font 회피 위해 `CTFontCopyFamilyName` 으로 *실제 family name* 검증, Windows 는 `DWriteFontCtx.isFontAvailable` 로 검증.

> **schema strict 검증** (macOS 만 — Windows 는 graceful, #118 step 3 에서 통일 예정):
...
> - Type mismatch (예: `width` 에 string) 면 fatal `type mismatch at "..."`.
> - 위 검증 모두 `validateStructure(user, default, ctx)` 한 함수가 재귀로 처리 — `DEFAULT_CONFIG_JSON` 과 user config 를 비교.
...
| Hanja 변환 (Option+Return) | 🟡 | [#166](https://github.com/ensky0/tildaz/issues/166) | NSTextInputClient 의 reconversion API (`attributedSubstringForProposedRange` 등) 미구현. Apple Terminal.app 도 동일 — `NSTextView` 만 작동. mac native 따름. enhancement 별 issue. |
...
>
> ghostty 가 Mode 2027 (grapheme cluster, [`macos_session.zig:114`](src/macos_session.zig#L114)) ON 으로 ZWJ cluster (man + ZWJ + woman + ZWJ + girl) 를 받으면 첫 base char (man) 만 cell 차지 (wide = 2 cells), 나머지 codepoint 들은 모두 *같은 cell 의 grapheme extras* 로 append 되고 cursor 안 advance — cluster 1 개 = grid 의 2 cells. 시각적으로는 우리 metal renderer 가 cluster 의 base + extras 를 모아 CTLine 으로 single shape → family ligature 정상 표시 ([`macos_metal.zig:469-487`](src/macos_metal.zig#L469-L487)).
>
...
| #166 | macOS Option+Return Hanja conversion — NSTextInputClient reconversion API |
...
*마지막 업데이트: 2026-05-10 (v0.4.0 — cross-platform unification + Win IME parity).
