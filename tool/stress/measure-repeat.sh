...
#
# 결과는 `--out` (기본 `tool/stress/shots/`) 에 `<phase>-raw.txt` (로그 원문) 와
...
usage() {
    cat <<'USAGE'
쓰는 법: tool/stress/measure-repeat.sh [옵션]
  ...
  --out <디렉터리>     결과 위치 (기본 tool/stress/shots)
...
[ -n "$OUT" ] || OUT="$REPO_ROOT/tool/stress/shots"
...
if [ "$PANES" != 1 ]; then
    ...
    RUNNER_CMD="powershell -NoProfile -ExecutionPolicy Bypass -File $(native_path "$REPO_ROOT/tool/stress/pane-runner_windows.ps1") $(native_path "$BARRIER") $(native_path "$STRESS")"
fi
    ...
    for w in $WORKLOAD_LIST; do
        ...
        if [ "$PANES" = 1 ]; then
            ...
            $RUN_TIMEOUT "$EXE" $INSTANCE_ARGS -e "$RUNNER_CMD $w $TILDAZ_STRESS_BYTES" -size 120x40 -scrollback "$SCROLLBACK" \
                >/dev/null 2>&1 &
            _app_pid=$!
            powershell -NoProfile -ExecutionPolicy Bypass -File "$(native_path "$REPO_ROOT/tool/stress/split-panes_windows.ps1")" -Panes "$PANES" 2>&1 | tr -d '\r'
            sleep 0.8
