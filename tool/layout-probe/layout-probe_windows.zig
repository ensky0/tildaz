...
//!
//! **Windows 전용이다.** macOS 는 [`tool/layout-probe/layout-probe_macos.m`](layout-probe_macos.m) 이
...
comptime {
    if (builtin.os.tag != .windows) {
        @compileError("layout-probe 는 Windows 전용이다. macOS 는 tool/layout-probe/layout-probe_macos.m 를 쓴다.");
    }
}
