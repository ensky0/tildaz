...
| Split panes | [#483](https://github.com/ensky0/tildaz/issues/483) | Slow track. Steps 1–3 of 6 landed with zero visible change (pixel A/B identical on all three platforms): `src/pane_layout.zig` (pure split tree + per-pane grid, hit testing, neighbor search), the three renderers split into `drawPane(PaneDraw)` / `endFrame`, `SessionCore` tabs became `TabGroup`s over that tree. Step 4a added the `[keys]` pane actions and `SessionCore` operations on every platform; steps 4b–4c wired the Linux host (split / focus / resize / equalize / zoom, gray separators + amber focus line, click-to-focus, separator drag, `…` menu items, `+` Alt-click). Next: macOS and Windows hosts (step 5), then IME / drain / per-pane scrollbar (step 6) |
