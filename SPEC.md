...
|---|---|---|---|---|---|---|---|
| Drop-down 위치 | 다른 모든 윈도우 위 | `WS_EX_TOPMOST` + `SetWindowPos(HWND_TOPMOST)` | `NSPopUpMenuWindowLevel` (101) | `zwlr_layer_shell_v1.get_layer_surface(layer=top)` + `keyboard_interactivity=exclusive` (L8-α). layer-shell 을 advertise 안 한 compositor (mutter / Muffin — GNOME / Cinnamon) 에선 xdg-shell fallback = 일반 창 (drop-down 아님). *수용된 한계가 아니라 추적 중 gap* — GNOME / Cinnamon 은 사용자 많아 v0.5.0 범위 ([#215](https://github.com/ensky0/tildaz/issues/215)) | ✅ | ✅ | ✅ (layer-shell 계열) / ❌ mutter 계열 ([#215](https://github.com/ensky0/tildaz/issues/215)) |
| 표시 모니터 (멀티모니터) | **마우스 커서가 있는 모니터**에 등장 (drop-down 표준 UX). show() / hotkey toggle show 시 적용 — work-area relayout(해상도/DPI 변화)은 현재 모니터 유지 | show() 시 `MonitorFromPoint(GetCursorPos)` ([window.zig:707](src/window.zig#L707)) | `NSScreen.mainScreen` — 커서 모니터로 정렬 예정 ([#240](https://github.com/ensky0/tildaz/issues/240)) | layer-shell 계열은 compositor 가 커서 출력에 anchor. mutter/muffin fallback 은 GNOME(#228)/Cinnamon(#229) extension 이 `get_current_monitor()`(커서 모니터)로 배치 | ✅ | 🟨 (mainScreen → 정렬 [#240](https://github.com/ensky0/tildaz/issues/240)) | ✅ (extension 계열) |
...
|---|---|---|---|---|---|---|
| 윈도우 토글 (drop-down) | F1 (`RegisterHotKey`) | F1 (CGEventTap, config 변경 가능) | F1 portal `GlobalShortcuts.BindShortcuts` ([70c41e9](https://github.com/ensky0/tildaz/commit/70c41e9), L9) + portal 미가용 환경에선 `tildaz --toggle` Unix socket IPC ([9803c62](https://github.com/ensky0/tildaz/commit/9803c62), #198) | ✅ | ✅ | ✅ |
