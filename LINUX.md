...
#### L8-δ 시연 추가 — fractional scaling 환경의 dock 위 1px 침범

KDE Plasma 6.6.5 (KWin) + fractional scaling (170%) 환경 검증 결과 — bottom dock
이 floating mode 일 때, tildaz 의 layer-shell surface 의 *bottom edge* 가 dock
의 floating margin 상단 1 physical pixel 만큼 침범. dock 의 실제 panel content
(아이콘 / clock 등) 은 안 가림, *floating margin 의 cosmetic 빈 공간 맨 위 1px*
만 살짝 덮음.

산술:
- `[Containments][46][General] thickness=46` logical * 1.7 ≈ 78 physical
- KWin 이 알려준 surface logical_h = 1225 → physical = 1225 × 1.7 = 2082.5
- xdg-shell window (Firefox 등) — KWin xdg-shell placement 가 일관 round 정책
  → dock 과 정확히 만남 (시각 갭 0)
- 우리 layer-shell + `wp_viewport.set_destination` — KWin layer-shell placement
  의 sub-pixel rounding 이 xdg-shell 과 비대칭 (ceil 쪽) → surface bottom physical
  ≈ 2083 → dock floating margin 상단 1px 침범

원인은 위의 *dock 일부 가림* 사례와 동일 — KWin 의 floating dock + layer-shell
조합 + fractional sub-pixel placement. 영향: 시각적으로 거의 인지 불가 (floating
margin 의 투명 영역 1px). dock content 정상.

#### 우회

- Plasma 의 dock "Floating" 옵션 끄기 (사용자 환경) — base height 만큼은
  정상적으로 exclusive zone 광고되므로 tildaz 도 정확히 회피.
- 또는 fractional scaling 100% / 200% 사용 — 정수배 scale 에선 sub-pixel
  placement 비대칭 사라짐.
...
- KWin 6.3+ 의 `xx_fractional_scale_v2` protocol 완성 + advertise 시 layer-shell
...
**KRunner (Alt+F2) / Application Menu / kstart 통해 실행** 해야 portal-kde 의
GlobalShortcuts 가 정상 동작.

이유: portal-kde 가 D-Bus sender pid 의 process tree 매칭으로 client app_id
식별. terminal 에서 직접 실행하면 부모 (Konsole 등) 의 desktop entry 로 잡혀
portal-kde 의 BindShortcuts 가 *Konsole 의 KGlobalAccel component 안에 등록*
→ 우리 tildaz process 에 trigger signal 안 옴.

`gio launch ~/.local/share/applications/tildaz.desktop` 같이 GIO / DE launcher
통하면 portal-kde 가 정확히 `app_id: "tildaz"` 인식. 첫 등록 시 KDE 의 user
dialog (`전역 단축키 요청됨 — TildaZ 에서 ... 등록`) 표시 — "확인" 누르면
KGlobalAccel runtime 에 등록 완료.

install 자동화 — `dist/linux/install.sh` 가 placeholder 치환 + icon (hicolor
scalable) + desktop database / icon cache refresh 를 best-effort 처리. binary
는 build 결과물 (`zig-out/bin/tildaz`) 또는 `--exe` 로 명시:
...
- `~/.local/share/applications/tildaz.desktop` (사용자 환경 절대경로로 치환된 Exec)
- `~/.local/share/icons/hicolor/scalable/apps/tildaz.svg` (`docs/favicon.svg` 출처
  — mac `AppIcon.icns` / Windows `tildaz.ico` 와 동일 cross-platform 아이콘)

`auto_start=true` 면 첫 실행 후 자동으로 `~/.config/autostart/tildaz.desktop`
도 작성됨 (L11-α, [src/autostart/linux.zig](src/autostart/linux.zig)).

uninstall: `bash dist/linux/uninstall.sh` — 위 세 파일 + autostart entry 삭제.
사용자 config (`~/.config/tildaz/`) 와 log (`~/.local/state/tildaz/`) 는 보존.

distro packaging (.deb / .rpm / AppImage / Flatpak) 의 system-wide install
흐름 (`/usr/bin/tildaz` 등) 은 별도 milestone (L11-δ deferred).
...
| KDE Plasma 6 portal-kde GlobalShortcuts silent skip — F1 trigger 안 됨 | [70c41e9](https://github.com/ensky0/tildaz/commit/70c41e9) | 두 가지 cause 가 묶임: (a) terminal child 환경에서 실행 시 portal-kde 가 app_id 를 부모 process (`org.kde.konsole` 등) 로 매칭 → KGlobalAccel 등록이 Konsole component 안으로 → 우리에 trigger 안 옴. KRunner / Application Menu / `gio launch` 통한 실행 시 `app_id: "tildaz"` 정상 인식 + 첫 등록 user dialog 표시. (b) [wlr-layer-shell spec](https://wayland.app/protocols/wlr-layer-shell-unstable-v1) 의 commit-only re-map sequence 가 KWin 에서 동작 X (configure event 미발신) — wl_surface + layer_surface 둘 다 destroy / recreate 가 정공. 진단 일지는 [#189 댓글 chain](https://github.com/ensky0/tildaz/issues/189). |
