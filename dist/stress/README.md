...
> ⚠️ **`-size` 로 띄운 창에서는 탭을 만들지 않아요** ([#506](https://github.com/ensky0/tildaz/issues/506)).
> 탭바가 생기면 세로 공간이 줄지만 격자는 요청값을 유지해서 (그 고정이 #382 의 목적이에요)
> 맨 아래 행이 창 밖으로 밀려요. 모든 탭이 같은 격자를 쓰므로 기존 탭도 함께 어긋나고,
> `Ctrl+L` 로 회복된 것처럼 보여도 격자는 그대로예요.
