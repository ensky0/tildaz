...
```sh
sh tool/mouse-probe/mouse-probe.sh              # ?1002 + ?1006 — 기본
sh tool/mouse-probe/mouse-probe.sh 1003         # hover (버튼 없는 이동) 까지
sh tool/mouse-probe/mouse-probe.sh 1000         # motion 없음 — 누름/뗌만
sh tool/mouse-probe/mouse-probe.sh 9 1006       # x10 — 누름만, modifier 없음, 좌표 223 상한
sh tool/mouse-probe/mouse-probe.sh --log /tmp/m.log   # 화면 + 파일 (자동 판정용)
```
...
```
"C:\Program Files\Git\bin\bash.exe" C:/Users/<you>/tildaz/tool/mouse-probe/mouse-probe.sh
```
...
선례는 [`tool/input_macos.m`](../input_macos.m) 예요.
