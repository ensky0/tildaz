...
| 윗줄 top | 셀 top (0) |
| 점선 (`4:4`) · 파선 (`4:5`) | **같은 경로.** 셀을 등분해 각 구간의 **앞 절반**을 칠한다 (칠한 폭 = 빈 폭). 구간 크기만 다르다 — 점선 `2 × 두께` (정사각형 dot), 파선 **셀 폭** (셀당 1 조각) |
...
**wide char 는 조각 수가 정확히 2 배**여야 밀도가 유지된다. 그래서 base 셀 기준으로 세고 `span` 을 곱한다 — 그냥 `round(셀폭 / 구간)` 하면 반올림 때문에 2 배가 깨진다 (구간 8 일 때 narrow 19 → 2 지만 wide 38 → 5). 어긋나면 한글 구간만 리듬이 달라진다.

**균등 배치가 되기까지 두 번 고쳤다** (2026-08-03 실기, 사용자 지적).
