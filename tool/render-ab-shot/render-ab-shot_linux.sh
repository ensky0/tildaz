...
# macOS 의 `tool/render-ab-shot/render-ab-shot_macos.sh` 에 대응한다 — #586 Linux atlas `grow` 실기에서 만들었다.
#
# ```sh
# tool/render-ab-shot/render-ab-shot_linux.sh <화면.sh> <격자> <대기초> <태그> <바이너리A> <바이너리B> [<바이너리C> …]
# tool/render-ab-shot/render-ab-shot_linux.sh /tmp/stack2.sh 150x40 8 stack2 /tmp/tildaz-4096 zig-out/bin/tildaz /tmp/tildaz-512
# ```
#
