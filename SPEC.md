...
| 이전 탭 | (없음 — Windows 미구현) | Shift+Cmd+[ | ❌ #125 | ✅ |
| 다음 탭 | (없음 — Windows 미구현) | Shift+Cmd+] | ❌ #125 | ✅ |

### 2.3 클립보드

| 동작 | Windows | macOS | Win | Mac |
|---|---|---|---|---|
| 복사 (단축키) | Ctrl+Shift+C | Cmd+C | ❌ #120 | ✅ |
| 복사 (드래그 selection 후 자동) | `selection.finish()` → `copyToClipboard` | (미구현) | ✅ | ❌ #122 |
| 붙여넣기 (단축키) | Ctrl+Shift+V | Cmd+V | ✅ | ✅ |
| 붙여넣기 (마우스 우클릭) | `WM_RBUTTONDOWN` → `pasteClipboard` (cmd.exe console 표준 패턴) | 동일 (`tildazRightMouseDown` → `handlePaste`) | ❌ #119 | ❌ #119 |
...
| 동작 | Windows | macOS | Win | Mac |
|---|---|---|---|---|
| Ctrl+C → SIGINT (\\x03) | `WM_CHAR` 자동 ASCII control char | (미전달 — 버그) | ✅ | ❌ #121 |
| Ctrl+A ~ Ctrl+Z 일반 control | 동일 | (미전달 — 매트릭스 점검) | ✅ | ❌ #121 |
...
| selection finish 후 자동 copy | cell 영역 | `selection.finish()` → `copyToClipboard` | (미구현) | ✅ | ❌ #122 |
| 우클릭 paste | 어디든 | `WM_RBUTTONDOWN` → `pasteClipboard` | `tildazRightMouseDown` → `handlePaste` | ❌ #119 | ❌ #119 |
| 휠 / 트랙패드 scroll | 셀 영역 | `WM_MOUSEWHEEL` → `scrollViewport` | `tildazScrollWheel` → 동일 | ✅ | ✅ |
| 스크롤바 클릭 + 드래그 | 우측 가장자리 | `mouse.x >= client_w - SCROLLBAR_W` → `scrollToY` | (미구현) | ✅ | ❌ #123 |
...
| 단일↔멀티 전환 시 cell 영역 동기화 | macOS 만: 단일 탭 시 탭바 자리 없음 → 전환 시 cell 영역 변화 → 모든 탭 cols/rows 재계산. Windows: 탭바 자리 항상 reserve 라 변화 없음 (해당 path 불필요). | (해당 없음 — 탭바 자리 항상) | `syncTerminalGeometry` 호출 | — | ✅ |
...
| `auto_start` | bool | `true` | (없음 — autostart 미구현) | ✅ | ❌ #126 |
| `hidden_start` | bool | `false` | (없음) | ✅ | ❌ #118 |
...
| SIGHUP 무시 셸 fallback | SIGKILL 강제 | (자동) | 미구현 (별도 후속, `nohup` wrapper 같은 rare 케이스만 hang) | ✅ | 🟨 |
...
| platform | 현재 config | 의도 spec |
|---|---|---|
| Windows | `<exe_dir>\config.json` (단일 — exe 옆) | `%APPDATA%\tildaz\config.json` |
| macOS | `~/Library/Application Support/tildaz/config.json` | `~/.config/tildaz/config.json` |

→ 양쪽 platform *위치 변경* 필요. 마이그레이션 정책은 이슈에서 결정 (기존 사용자의 config 자동 이동? 첫 실행 시 안내?).

### 11.2 Open Config / Log 단축키 (default editor 열기)

| 동작 | Windows | macOS | Win | Mac |
|---|---|---|---|---|
| Config 열기 | Ctrl+Shift+P | Shift+Cmd+P | ❌ #128 | ❌ #128 |
| Log 열기 | Ctrl+Shift+L | Shift+Cmd+L | ❌ #128 | ❌ #128 |
...
| 항목 | 우선순위 | 이슈 | 비고 |
|---|---|---|---|
| Ctrl+key PTY 전달 (Ctrl+C SIGINT 등) | 🔴 critical | #121 | Windows `WM_CHAR` 자동, macOS 미전달 |
| 드래그 selection 자동 copy | 🟡 critical | #122 | Windows `selection.finish()` 자동 |
| 마우스 우클릭 paste (양쪽 변경) | 🟡 critical | #119 | Windows 현재 가운데 버튼 (`WM_MBUTTONDOWN`, deprecated) → 우클릭 (`WM_RBUTTONDOWN`) 으로 변경. macOS 우클릭 추가. |
| 스크롤바 마우스 클릭 + 드래그 | 🟡 critical | #123 | Windows `mouse_down` 의 scrollbar 영역 분기 |
| autostart (LaunchAgent) | 🟡 | #126 | 옛 #73 의 `src/macos/autostart.zig` 참고 |
...
| 단일 탭 시 탭바 자리 reserve 버그 | 🟡 | #127 | Windows 가 *탭바 자리 항상* — macOS 와 동일하게 단일 탭 시 자리 없게 정정. |
