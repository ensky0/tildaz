...
#
# ```powershell
# tool\tab-churn-check_windows.ps1 -Probe               # ① 조건 확인 (단축키가 먹는지) — 먼저 돌린다
# tool\tab-churn-check_windows.ps1                      # ② 본 회차 20 사이클 · 간격 1.5 초
# tool\tab-churn-check_windows.ps1 -Cycles 10 -GapMs 2000
# ```
#
# ## 함정 — 빠뜨리면 반대 결론이 나온다
#
...
#    하나 그렇게 버렸다 (`tool/send-keys_windows.ps1` 이 같은 주의를 적어 두었다).
...
public static class TzWin
{
    ...
    // `wScan` 을 반드시 채운다 (`tool/send-keys_windows.ps1` 의 같은 주석) — scan code 가 0 이면
