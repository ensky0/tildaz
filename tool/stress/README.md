...
두 검사는 **그 기기에 해당할 때만** 걸려요 (2026-08-16 · 미니PC Firebat ZY-A8 실측으로 고쳤어요).
데스크탑처럼 배터리가 아예 없으면 (`/sys/class/power_supply/` 가 빔) AC 검사를 건너뛰고
`power=AC (배터리 없음)` 으로 찍어요 — 경고의 취지가 *배터리 스로틀링 · 패널 강등*인데 그 위험이
없으니까요. 주사율 최대는 **지금 쓰는 해상도 안에서만** 찾아요 — 모든 모드에서 찾으면
`3840x2160@60` 화면이 `1280x1024@75.03` 때문에 늘 걸려요 (해상도를 바꾸는 건 측정 조건을 바꾸는
것이라 해법이 못 돼요). 고치기 전에는 이 두 오탐만으로 `hygiene_check` 가 **그 기기에서 항상
실패**했어요.
...
tool/stress/measure-repeat.sh --phase before
tool/stress/measure-repeat.sh --phase after --workloads zwj,plain
tool/stress/measure-repeat.sh --phase win-pane8 --panes 8 --workloads plain   # 실제 앱 8 pane (Windows · 아래)
```
...
```sh
tool/stress/compare-terminals.sh --mb 64 --workload plain --repeat 5
```
...
```sh
tool/stress/check-input-loss_linux_macos.sh                 # 기본 — 10 회 · plain · 2 GiB
tool/stress/check-input-loss_linux_macos.sh --presses 20
```
...
⚠️ **macOS 는 회차 폭이 커서 절사평균만 보면 안 돼요** ([#476](https://github.com/ensky0/tildaz/issues/476)).
앞 세 회차 (유휴 평균 0.63~0.74 ms) 와 뒤 두 회차 (2.39 · 2.64 ms) 가 **두 무리로 갈려요.** 회차가
갈수록 나빠지는 모양이라 열 누적이 의심되지만 **확인하지 않았어요.** 유휴 최악의 폭 (0.79~16.58 ms) 은
**Linux 값 12.35 ms 를 품어서**, 이 표만으로 "macOS 가 유휴 최악에서 낫다" 고 말할 수 없어요.
다섯 회차 모두 표본 30 / 30 · 에코 30 B 로 폐기 장치를 통과했으니 **알려진 오염은 아니에요.**
...
#### 지연을 구간과 분포로 읽는 법 (`in_gate` · `hist`)

평균과 최악만으로는 못 가르는 두 물음이 있어서 계측을 넣었어요
([#473](https://github.com/ensky0/tildaz/issues/473) · [#481](https://github.com/ensky0/tildaz/issues/481)).
`perf` 덤프에 네 줄이 함께 나와요.

```
input    samples=31 ms=43.334 max_ms=9.842     ← 키 → 첫 present 완료 (합)
in_gate  samples=31 ms=17.123 max_ms=1.734     ← 그중 **프레임을 기다린 몫**
hist     in={ 13, 16, 1, 0, 1, 0 } out={ … }   ← 분포: <1 · 1-2 · 2-4 · 4-8 · 8-16 · >16 ms
histgate in={ 29, 2, 0, 0, 0, 0 } out={ … }    ← 게이트의 분포
```

- **`input − in_gate` 가 "깨어난 뒤" 몫**이에요 (렌더 · present · 셸 왕복). 늦은 곳이 *깨우기*
  인지 *그 뒤*인지는 이 뺄셈으로 갈려요.
- **`hist` 는 "하나가 튀었나 여럿이 퍼졌나" 를 가려요.** 위 예에서 유휴 최악 9.84 ms 는
  `8-16` 버킷의 **1 개**가 만든 값이고 나머지 29 개는 2 ms 미만이에요 — *하나만* 튄 거예요.
  같은 회차의 `histgate` 가 전부 2 ms 미만이라 그 튐은 **깨우기 탓이 아니에요.**
- **Linux 는 이 계측의 대조군**이에요. 그쪽은 poll 반복에서 곧바로 그리므로 `in_gate` 가 0 에
  가까워야 하고, 그렇지 않으면 계측 자체를 의심해요.
- 버킷 경계를 2 의 거듭제곱으로 둔 이유는 60 Hz 의 **반 프레임 (8.3) 과 한 프레임 (16.67)** 이
  경계에 오게 하기 위해서예요 — "프레임 주기가 상한" 이라는 주장을 버킷 하나로 읽을 수 있어요.
- **`present` 를 같은 회차에서 함께 봐요.** *"지연의 하한은 present 가 vsync 를 기다리는 것"* 은
  자연스러운 가설인데, 2026-09-05 실측에서 **반증됐어요** — `present` 평균이 유휴 **0.67 ms** ·
  폭포 **0.18 ms** 이고 `swapwait` 이 0 이라 기다린 흔적이 없어요. 폭포의 게이트 (7.73 ms) 는
  present 가 아니라 **프레임 클럭 주기** 대기예요 (`onrender` 가 4.14 ms 인데 주기는 16.9 ms).
- **`pty_rt` 가 셸 왕복이에요** (PTY write → 그 뒤 첫 출력 도착). `input` 이 그 구간을 포함하므로,
  이 값이 있어야 *"남는 몫이 자식 쪽인지"* 를 뺄셈이 아니라 관측으로 가를 수 있어요. 실측
  (2026-09-05 · `cmd.exe`) 은 **평균 0.23 ms · 최악 0.55 ms · 40 표본 전부 1 ms 미만**이었어요 —
  **자식 왕복은 지연의 범인이 아니에요.**

그래서 유휴 지연이 네 조각으로 완전히 분해돼요 (같은 회차 · 평균).

| 구간 | 값 | 비중 |
|---|---|---|
| `pty_rt` (셸 왕복) | 0.23 ms | 18 % |
| `in_gate` (프레임 대기) | 0.69 ms | 54 % |
| 렌더 · present · 큐 (나머지) | 약 0.36 ms | 28 % |
| **`input` 합계** | **1.28 ms** | 100 % |

⚠️ **유휴 최악의 튐은 회차마다 나타나지 않아요.** 같은 기기 · 같은 설정에서 1 회차는 최악 9.84 ms
(`8-16` 버킷 1 개) 였는데 2 회차는 **2.20 ms** 로 전 표본이 4 ms 미만이었어요. 한 회차만 보고
"있다 / 없다" 를 판정하지 말고, 튄 회차의 **분포와 게이트를 함께** 적어요.

#### ⚠️ 조각의 최악은 **서로 다른 표본**에서 나와요 — 합계로 검산하지 말아요

이 계측의 가장 헷갈리는 점이에요. 카운터가 조각마다 **독립적으로** 최악을 들기 때문에, `input`
최악이 8.8 ms 인 **그 표본**에서 각 조각이 얼마였는지는 이 값들로 알 수 없어요. 그래서 조각의
최악을 다 더해도 `input` 최악에 못 미치는 일이 흔해요 — **모순이 아니라 표본이 다른 것**이에요.

```
input  max 8.795   ← 어떤 표본
itw    max 0.495   ← 다른 표본일 수 있음
pty_rt max 0.476   ← 또 다른 표본일 수 있음
in_gate max 1.369  ← 합계 3.0 ms 지만 input 최악은 8.8 ms
```

- **평균은 검산해도 돼요** (같은 표본 집합의 합이니까). 유휴 실측이 그래서 깔끔하게 나눠졌어요 —
  1.28 ms = `in_gate` 0.69 + `pty_rt` 0.23 + 나머지 0.36.
- **최악은 검산하면 안 돼요.** *"튄 그 한 표본이 어디서 늦었나"* 를 보려면 **한 표본의 시각들을
  함께 남기는 per-sample 기록**이 필요하고, 그건 계측이 무거워져 측정 대상을 바꿀 위험이 있어요
  ([#481](https://github.com/ensky0/tildaz/issues/481) 에서 그 이유로 착수하지 않았어요).

**20 회차 (2026-09-05) 로 배제된 원인 넷** — 유휴 최악의 튐 (8~17 ms · 20 회차 중 6 회) 은 아래
어느 것도 아니에요. 다시 볼 때 이 목록에서 출발해요.

| 후보 | 배제 근거 |
|---|---|
| 깨우기가 늦다 | `in_gate` 가 전 표본 2 ms 미만 |
| present 가 vsync 를 기다린다 | `present` 0.18~0.67 ms · `swapwait` 0 |
| 자식 셸 왕복 | `pty_rt` 40/40 이 1 ms 미만 |
| 키 → write 큐 | `itw` 40/40 이 1 ms 미만 |

단 **한 표본은 설명돼요** — 어느 회차의 `in_gate` 최악이 15.99 ms 였고, 이건 59 Hz 한 프레임
(16.9 ms) 을 통째로 놓친 것이에요.
...
위 표는 **소스 구조**예요. 여기서 *"키가 프레임 주기 안 아무 때나 도착하니 Windows 유휴 평균에는
반 프레임 (8.3 ms) 이 얹힌다"* 는 설명을 오래 달아 두었는데, **2026-09-05 에 그 대기를 직접 재서
반증했어요.**

> ⚠️ **유휴에서는 반 프레임 대기가 일어나지 않아요.** `in_gate` (키 도착 → 프레임 진입) 를 새로
> 재 보니 **평균 0.55 ms · 최악 1.73 ms** 였고, 분포도 31 표본 중 **29 개가 1 ms 미만**이에요
> (노트북 i5-1240P · 59 Hz · `perf` 덤프의 `in_gate` · `histgate`).
> 반 프레임의 6.6 % 라 그 설명으로는 값이 안 나와요. 게이트가 반 프레임을 얹는 것은 **폭포 중**이에요
> — 같은 회차의 폭포에서 `in_gate` 가 **평균 7.26 ms** (입력 지연 10.80 ms 의 67 %) 였어요.

그리고 근거였던 유휴 9.79 ms 자체가 **옛 값**이에요. [#439](https://github.com/ensky0/tildaz/issues/439)
· [PR #480](https://github.com/ensky0/tildaz/pull/480) 뒤로 유휴 평균은 **1.40 ms** 이고
([#481](https://github.com/ensky0/tildaz/issues/481) 이 기록한 1.45 ms 와 같아요), 그중 게이트는
0.55 ms 예요. 아래 macOS 서술도 이 전제 위에 있으니 함께 읽어요.
...
경로만** 봐요. 한글 입력의 응답 지연을 재려면 계측 지점을 IME 경로에도 놓아야 해요. 세 platform 이
서로 다른 것을 재고 있어서 **지금 이 문서의 숫자는 전부 영문 경로 값이에요** —
[#475](https://github.com/ensky0/tildaz/issues/475) 에서 따로 다뤄요.
...
> ⚠ **아래 "기록 수치" 는 고치기 *전* 값이에요.** #439 의 (a)(b)(c) 로 세 host 가 PTY 도착을
> 통보받게 되면서 **세 host 모두 fix 후 실측을 마쳤어요** (각각 같은 세션 A/B). 유휴 평균이
> **Linux 8.80 → 0.64 ms · macOS 5.83 → 0.94 ms · Windows 8.33 → 1.45 ms**, 최악이
> **15.82 → 3.69 · 9.51 → 3.76 · 19.49 → 9.17 ms** 로 내려갔어요. 기록 수치는 *그 fix 의 값을 재는
> 기준선*으로 남겨 둬요. fix 후 수치와 회귀 검사는 [Linux](#fix-후-실측--linux-2026-08-16) ·
> [macOS](#fix-후-실측--macos-2026-08-16) · [Windows](#fix-후-실측--windows-2026-08-16) 절에 있어요.

```sh
tool/stress/measure-idle-latency.sh                 # 20 회 · 간격 2 초
tool/stress/measure-idle-latency.sh --samples 40 --gap 3
```
...
**다만 실측에서는 N+1 과 N+2 가 섞여 나와요** — 2026-08-16 회차에서 21 과 22 가 번갈아 나왔고
**fix 없는 대조군에서도 같았어요**. 자세한 것은 [fix 후 실측 — Windows](#fix-후-실측--windows-2026-08-16)
절의 마지막 소절에 있어요.
...
### fix 후 실측 — Linux (2026-08-16)

미니PC Firebat ZY-A8 (AMD Ryzen 7 8845HS · 8C/16T) · CachyOS · **KDE Plasma Wayland** ·
3840x2160 **60 Hz** (scale 1.7 → 논리 2259x1271) · `render_path=gpu-gl` · AC · `hygiene_begin` 적용 ·
ReleaseFast + SIMD. 대조군은 fix 커밋의 `src/` 변경을 **역적용해 다시 빌드한 것**이에요 (같은 세션
A/B — 위 "기록 수치" 는 다른 날 값이라 A/B 로 쓰지 않았어요).

**통보가 실제로 붙었는지는 로그로 갈라요** — `[startup] output wake installed (idle PTY notify)`
가 있으면 fix 빌드, 없으면 대조군이에요. **세 host 가 글자까지 같은 줄**이라 어느 머신에서 잰
로그든 같은 방법으로 갈라요. 값이 안 좋을 때 *"fix 가 안 붙은 것"* 과 *"붙었는데 효과가 없는 것"* 을
가르려면 이 줄이 필요해요.

| | 대조군 (fix 없음) | fix 후 |
|---|---|---|
| **유휴 평균** | 8.80 ms (8.78~8.82) | **0.64 ms** (0.55~0.70) — **13.7 배** |
| **유휴 최악** | 15.82 ms (15.54~16.10) | **3.69 ms** (2.52~5.38) — 4.3 배 |
| 폭포 입력 평균 (회귀 검사) | 10.68 ms (10.34~11.92) | 11.26 ms (10.79~12.08) |
| 폭포 입력 최악 | 18.47 ms (18.07~18.95) | 18.69 ms (17.85~21.59) |
| **`onrender` 회당** (회귀 지문) | 0.147 ms (0.138~0.153) | **0.142 ms** (0.128~0.145) |
| `drain` (폭포) | 16589 회 / 10716 ms | 16577 회 / 10726 ms |
| `readloop bytes` (폭포) | 1,091,219,410 | 1,084,844,479 |

유휴 fix 후 5 회 · 유휴 대조군 2 회 · 폭포는 양쪽 5 회씩이고, 5 회 회차는 **절사평균 + min~max**
예요. 유휴는 전 회차 **표본 20/20 · 입력 혼입 0** 이라 폐기 회차가 없어요.

**프레임 주기 종속이 끊어졌어요.** 대조군 최악 15.54~16.10 ms 는 `frame_poll_ms` **16 ms** 에 그대로
붙어 있어요 (#439 가 잰 fix 전 값 8.71 / 16.39 와 같은 자리예요). fix 후 최악 5.38 ms 는 그 주기와
아무 관계가 없어요.

#### Linux 는 유휴 게이트가 없어도 폭포가 안 나빠져요 — 소스 판정이 실측으로 확인됐어요

macOS · Windows 는 통보가 드레인 자리를 빼앗아 폭포 입력 지연이 **5 배** 나빠지는 회귀가 있었고
(아래 [회귀 검사](#-회귀-검사에-폭포-모드를-반드시-함께-돌려요) 절), 그래서 유휴 게이트를 넣었어요.
Linux 에는 그 게이트가 없어요 — 메인 루프가 iteration
마다 드레인 + 렌더라 통보가 `poll` 을 일찍 깨워도 *자리* 가 안 바뀐다는 판정이었는데, 이번 A/B 가
그 판정을 확인했어요.

- **회귀의 지문인 `onrender` 회당 시간이 안 늘었어요** (0.147 → 0.142 ms). macOS 회귀 때는 같은
  값이 **0.31 → 8.3 ms** 로 27 배 뛰었어요.
- `drain` · `render` 총 시간도 그대로예요 (`render` 344.67 → 344.63 ms). 처리량도 그대로고요
  (`readloop bytes` −0.6 %).
- 폭포 입력 평균의 **+0.57 ms 는 회차 변동과 구분되지 않아요** — 대조군 자체의 회차 폭이 1.58 ms
  (10.34~11.92) 로 두 군의 차이보다 크고 두 분포가 겹쳐요 (Mann–Whitney U = 4, 양측 p ≈ 0.10).
  최악값도 18.47 → 18.69 ms 로 사실상 같아요.

### fix 후 실측 — macOS (2026-08-16)

[#439](https://github.com/ensky0/tildaz/issues/439) 의 (a)(b)(c) (PTY 도착 통보) 를 넣은 뒤 **같은
세션에서 A/B** 로 쟀어요 (같은 머신 · 같은 빌드 옵션 · `hygiene_begin` 적용). 대조군은 fix 를
`git stash` 로 빼고 다시 빌드한 것이에요 — 위 "기록 수치 ③" 은 다른 날 값이라 A/B 로 쓰지 않았어요.

| | 대조군 (fix 없음) | fix 후 |
|---|---|---|
| **유휴 평균** | 5.83 ms | **0.94 ms** (6.2 배) |
| **유휴 최악** | 9.51 ms | **3.76 ms** (2.5 배) |
| **폭포 입력 평균** (회귀 검사) | 1.51 ms | **1.43 ms** |
| `readloop bytes` (폭포) | 1,055,769,869 | 1,054,335,640 |
| `drain` (폭포) | 20520회 / 1125 ms | 20526회 / 1120 ms |

프레임 주기 (8.33 ms) 종속이 끊어졌어요 — fix 전 최악 9.51 ms 는 정확히 `주기 + render + present`
였는데 지금은 그 관계가 없어요. 처리량도 그대로예요 (10억 바이트를 같은 시간에 소화).

### fix 후 실측 — Windows (2026-08-16)

노트북 · Intel Core i5-1240P · 내장 패널 1920x1080 · **60 Hz** · 측정 위생 적용. 위 "기록 수치 ②"
와 **같은 하드웨어**예요. 주사율 근거는 앱 로그의
`[startup] frame clock started: refresh=60Hz period=16.67ms` 이고 `window initialized: dpi=96`
...
| | 대조군 (fix 없음 · 2 회) | fix 후 |
|---|---|---|
| **유휴 평균** | 8.33 ms (7.54~9.12) | **1.45 ms** (1.41~1.79) — **5.7 배** |
| **유휴 최악** | 19.49 ms (17.29~21.69) | **9.17 ms** (6.60~13.75) — **2.1 배** |
| **폭포 입력 평균** (회귀 검사) | 11.40 ms (11.38~11.41) | **9.80 ms** (9.69~10.41) |
| **`onrender` 회당** (자리 이동 검사) | 3.94 ms (2575.337 / 654) | **3.95 ms** (2598.222 / 658) |
| `readloop bytes` (폭포) | 2,174,328,760 | 2,174,328,760 |
| `drain` (폭포) | 33751회 / 7100 ms | 33741회 / 7106 ms |

fix 후 값은 유휴 5 회 · 폭포 3 회의 절사평균이고 (폭포는 3 회라 중앙값), 대조군은 각 2 회의
평균이에요. 회차별 유휴 평균 `1.41 · 1.79 · 1.41 · 1.46 · 1.49` / 최악
`6.60 · 13.75 · 10.49 · 8.48 · 8.53`, 폭포 평균 `10.41 · 9.69 · 9.80`.

**`onrender` 회당 시간이 대조군과 같아요 (3.94 vs 3.95 ms).** 아래 "회귀 검사" 절이 경고하는
*드레인 자리 이동*이 Windows 에서 일어나지 않는다는 뜻이고, 곧 `Window.frame_idle` 게이트가 실제로
돈다는 직접 증거예요. 폭포 입력 평균도 나빠지지 않았어요 (오히려 11.40 → 9.80 ms). 처리량도
그대로예요 — 같은 21.7 억 바이트를 같은 `drain` 시간에 소화했어요.

**배선은 로그로 확인했어요.** 이 세션에서 `host/windows.zig` 에 `[startup] output wake installed
(idle PTY notify)` 를 추가했고 (macOS 에만 있던 줄을 Linux · Windows 에도 넣어 세 host 대칭),
fix 회차 8 번 모두에 찍혔어요. 대조군 회차에는 없어요.

#### ⚠ 유휴 최악은 프레임 주기 종속이 끊겼지만 산포가 남아요 — 원인 미확정

대조군 최악 17.29~21.69 ms 는 프레임 주기 16.67 ms 를 따라가는 모양이었는데, fix 후에는
6.60~13.75 ms 로 그 관계가 사라졌어요. 다만 macOS (3.76 ms) 처럼 낮게 수렴하지는 않고 회차마다
2 배 넘게 흔들려요. 통보 자체는 붙어 있고 평균이 1.45 ms 라 *깨우기* 는 즉시 일어나는데, 최악
회차에서 무엇이 끼는지는 **이번 측정으로 갈리지 않았어요** (`PostMessageW` → `WaitMessage` 사이의
스케줄러 지연으로 보이지만 확인 필요).

#### 표본은 N+1 이 아니라 **N+1 ~ N+2** 로 잡혀요

위 "Windows 는 표본이 N + 1 로 잡혀요" 절이 근거로 든 ConPTY 시작 출력은 그대로인데, 실측에서는
21 과 22 가 섞여 나왔어요 (fix 회차 `22 · 21 · 22 · 22 · 21`, **대조군도 `22 · 21`**). 대조군에서도
같이 나오므로 이 통보 fix 와 무관한 기존 동작이에요. 어느 출력이 두 조각으로 나뉘어 세어지는지는
확인하지 않았어요.

#### ⚠ 회귀 검사에 **폭포 모드를 반드시 함께** 돌려요

유휴 지연만 재고 끝내면 **정반대 회귀를 놓쳐요.** 첫 구현은 도착할 때마다 통보했는데, 유휴는
좋아지는데 **폭포 중 입력 지연이 1.5 → 8.2 ms 로 5 배 나빠졌어요.**

원인은 통보 비용이 아니라 **드레인이 일어나는 자리가 바뀐 것**이에요. Windows 의 `idle_drain_fn`
은 `PeekMessage` 가 **빈 순간**에만, macOS 의 `idleDrainObserver` 는 run loop 이 **잠들기 직전**
(`kCFRunLoopBeforeWaiting`) 에만 돌아요 (SPEC §13.1). 통보를 계속 보내면 큐가 비지 않고 run loop 이
잠들지 않아 그 드레인이 굶고, 프레임 밖에서 하던 일이 통째로 프레임 안으로 밀려들어요.

**로그가 그것을 정확히 보여줘요** — `render` 와 `drain` 의 *총 시간은 그대로인데* `onrender` 만
**272 → 7212 ms** (회당 0.31 → 8.3 ms) 로 뛰었어요. 일이 는 게 아니라 자리가 바뀐 거예요. 그래서
회귀를 볼 때는 `onrender` 회당 시간을 함께 봐요.

```sh
tool/stress/measure-idle-latency.sh --samples 20 --gap 2        # ① 유휴 (본 목표)
tool/stress/measure-input-latency.sh --mode flood --presses 30  # ② 폭포 (회귀)
```

**`zig build stress` 로는 이 변경을 검증할 수 없어요** — `stress.zig` 는 host 없이 자체 세션을
만들어 `setOutputWake` 를 부르지 않으므로 통보 경로를 아예 안 타요. 처리량 회귀는 위 ② 회차의
`readloop bytes` · `drain` 으로 봐요.
...
```sh
tool/stress/measure-idle-cstates_linux.sh              # root 불필요 — core-idle 잔류율 (경향만)
sudo tool/stress/measure-idle-power_linux.sh           # 패키지 전력 (RAPL·turbostat) — 판정은 이쪽
```
...
```sh
tool/stress/compare-terminals.sh --mb 64 --workload plain --cols 120 --rows 40
```
...
> **`-size` 로 띄운 창에서 탭을 만들면 창이 그만큼 커져요** ([#506](https://github.com/ensky0/tildaz/issues/506)).
> 탭바는 세로 공간을 먹지만 요청한 격자는 기준값이라 (그 고정이 #382 의 목적이에요) 창을
> 아래로 키워서 격자를 지켜요. 탭을 닫으면 원래 크기로 돌아와요.
>
> 그래서 **요청 격자를 지킬 수 없는 화면에서는 아예 시작하지 않아요.** 판정은 *격자 +
> 탭바* 기준이라, 시작만 됐으면 탭을 만들어도 안전해요. 못 지키는 경우는 두 가지이고 둘 다
> stderr 에 이유를 적고 종료 코드 2 로 끝나요 (모달을 띄우면 스크립트가 멈추니까요).
>
> | 언제 | 어떻게 보이나 |
> |---|---|
> | 요청 격자 + 탭바가 화면 작업 영역보다 큼 | `"-size 120x40" does not fit this screen.` — 필요한 px 과 화면 px 을 함께 적어요 |
> | layer-shell 경로를 안 타는 데스크톱 (GNOME · Cinnamon · **sway**) | `"-size" cannot be used on this desktop.` — 거기선 창 크기를 compositor 가 정해서 `-size` 가 창에 영향을 못 줘요 |
>
> 두 번째는 **원래부터 그랬던 것이 드러난 것**이에요. 예전에도 그 데스크톱에서는 창과 격자가
> 어긋난 채 돌았는데 겉으로 표가 나지 않았어요. **측정은 KDE Plasma · Hyprland · COSMIC
> 에서 하세요.**
>
> sway 가 저 목록에 있는 것이 뜻밖일 수 있어요. sway 는 `zwlr_layer_shell_v1` 을 내주지만
> **우리가 일부러 안 써요** — `on_demand` layer surface 에 map 시 키보드 포커스를 주지 않아
> 토글 직후 타이핑이 안 되기 때문이에요 ([#454](https://github.com/ensky0/tildaz/issues/454)).
> 그래서 sway 는 GNOME · Cinnamon 과 같은 xdg_toplevel 경로이고, 창 크기는 i3 IPC 의
> `for_window` 규칙이 정해요.
...
```sh
tool/stress/compare-terminals.sh --mb 8 --workload zwj --repeat 1 --timeout 30 --capture
```

**경로는 선택이에요.** 안 주면 **`tool/stress/shots/`** 에 남아요 — 스크립트 바로 옆이라 찾기
...
|---|---|---|---|
| **macOS** | [`tool/color-capture_macos.m`](../color-capture_macos.m) (ScreenCaptureKit) | **창 단위** | **완전히 가려진 창도 찍혀요** (실측: 같은 자리에 창 둘을 겹치고 아래 창을 찍으니 아래 창 내용이 나왔어요). 스크립트가 `clang` 으로 빌드해서 써요. **화면 기록 권한**이 필요하고 잠금 화면이면 실패해요. 찾는 기준은 **bundle identifier** 예요 (`com.apple.Terminal` 등) — 앱 이름은 **시스템 언어로 번역**돼서 기준이 될 수 없어요 (아래 참고). 창을 못 찾으면 `screencapture -x` 전체 화면으로 물러서고 `?` 를 찍어요 |
...
PATH="$FP" tool/stress/compare-terminals.sh --mb 64 --workload zwj --repeat 3 --timeout 30
```
