...
>
> 모든 명시 폰트 (primary + fallback) 가 system 에 register 되어 있어야 함 — 하나라도 없으면 fatal dialog (`font_validate` 의 공통 메시지, chain dump + 미설치 표시 + config 경로). macOS substitute font 회피 위해 `CTFontCopyFamilyName` 으로 *실제 family name* 검증, Windows 는 `DWriteFontCtx.isFontAvailable` 로 검증한다. Linux 는 boot 시 `font_linux.familyInstalledDetail` 로 검증한다 (검증 시점·overlay 는 §7 startup shell 검증과 동일 C2 패턴; [#289](https://github.com/ensky0/tildaz/issues/289) B6, [#305](https://github.com/ensky0/tildaz/issues/305)). **검증과 실제 로드가 같은 해석 함수 (`resolveRequested`) 를 쓴다** — 둘이 각자 판정하면 검증을 통과하고도 다른 폰트가 조용히 그려질 수 있다 ([#409](https://github.com/ensky0/tildaz/issues/409)). 이름 하나를 face 하나로 해석하는 순서는 다음과 같고, 이름 비교는 모두 정규화 (소문자 + 공백 · `-` · `_` 제거) 후 exact match 다.
>
> 1. 적은 이름 그대로 `FC_FAMILY` 매치 — 반환된 primary family 와 alias 항목 중 하나와 맞으면 설치로 인정. `monospace` / `sans-serif` / `serif` generic family 는 substitution 이 의도이므로 항상 통과.
> 2. 설치된 family 목록 (`FcFontList`, 프로세스당 1 회) 에 정규화가 같은 이름이 있으면 **그 정식 표기로 다시 조회**한다 — fontconfig 의 family 매칭은 `-` 를 유효 문자로 보므로 `NotoSerifKannada-Light` 같은 표기는 1 에서 엉뚱한 폰트로 간다. 정식 표기가 원문과 같은데도 다른 폰트가 왔으면 시스템 별칭 규칙이 가로챈 것이다 (`Noto Color Emoji` → `Twemoji`) — **시작을 막지 않고** 그 폰트로 띄우며 로그를 남긴다 ([#406](https://github.com/ensky0/tildaz/issues/406)).
> 3. family 가 아니면 `FC_POSTSCRIPT_NAME` 으로 조회한다 (`NotoSansCJKkr-Regular`). `FcFontMatch` 는 없는 이름에도 최선 폰트를 돌려주므로 반환된 `postscriptname` 을 되짚어 검증한다. PostScript 이름은 family 가 아니라 **face** 를 가리키므로 bold PostScript 이름은 bold face 로 로드된다 — macOS `CTFontCreateWithName` 과 같은 의미다. Windows 는 아직 family 전용이다 (#409).
> 4. 셋 다 아니면 미설치 — fatal.
>
> 해석 결과의 `path` 와 함께 **face `index` 도 `FT_New_Face` 로 넘긴다** — `.ttc` / `.otc` 는 한 파일에 face 가 여러 벌이라 index 를 빼면 요청과 다른 face 가 열린다 (`Noto Sans CJK KR` 은 `NotoSansCJK-Regular.ttc` 의 index 1 이고 index 0 은 JP 다; [#428](https://github.com/ensky0/tildaz/issues/428)). 같은 이유로 face 동일성 판정 (chain dedup · styled 변종의 "regular 와 같은 파일" 검사) 도 (path, index) 쌍으로 한다. libfontconfig 자체를 못 여는 환경은 판정 불가로 두고 loader 의 에러 경로에 맡긴다 (미설치 오판 방지).
>
