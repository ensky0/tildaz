...
# macOS 의 `tool/deadkey-check/deadkey-check_macos.sh` 에 대응한다.
#
# ```powershell
# tool\deadkey-check\deadkey-check_windows.ps1                                   # zig-out\bin\tildaz.exe · US-International
# tool\deadkey-check\deadkey-check_windows.ps1 -Bin C:\path\tildaz.exe -Keep      # layout 을 세션에 남겨 둔다 (직접 쳐 볼 때)
# ```
#
# 무엇을 하나 —
...
#    아니면 그 자리에서 멈춘다 (`tool/send-keys_windows.ps1` 과 같은 규칙 — 합성 키는 포커스된 창으로 간다).
