...
# 메시지는 쉽고 간결하게 — **가장 중요한 규칙 중 하나**

바로 위의 언어 규칙 다음으로 먼저 지켜요 (2026-09-20 사용자 지시: *"모든 메시지는 쉽고 간결한
말이 좋아 … 가장 중요한 것 중에 하나야"*).

**대상은 우리가 쓰는 모든 글이에요** — 사용자에게 하는 답변, GitHub 이슈 · 댓글 · PR, 커밋
메시지, 릴리즈 노트, 앱이 보여 주는 텍스트. 한국어든 영어든 같아요.

- **문장을 짧게 끊어요.** 한 문장에 한 가지만 담아요.
- **읽는 사람이 몰라도 되는 내부 사정은 빼요.** 함수 이름 · 필드 이름 · 코드 구조는 그것이
  근거일 때만 적어요. 2026-09-20 [#642](https://github.com/ensky0/tildaz/issues/642) 답변에
  *"공통 함수 하나가 창 뷰포트만 본다"* 라고 적었다가 지적받았어요 — 신고자에게 필요한 것은
  *"모든 OS · 데스크톱에서 같다"* 뿐이었어요.
- **숫자도 근거일 때만 적어요.** *"20 열보다 좁으면"* 처럼 읽는 사람이 쓸 일 없는 수치는 빼요
  (같은 날 지시: *"더 쉽게 고치자. 숫자 빼고"*).
- **쉬운 말을 골라요.** *"fall apart"* 보다 *"break"*, *"마지막 줄보다 한 줄 위"* 보다
  *"마지막 줄 바로 위"*.

**짧게 쓰는 것과 근거를 빼는 것은 달라요.** 출처 링크와 이슈 번호는 그대로 남겨요 (아래
`# 문서화`). 줄이는 대상은 *설명하는 말*이지 *확인할 수 있는 근거*가 아니에요.
...
- **그 기기가 지금 없으면 브랜치만 올리고 기다려요.** 남은 OS 에서 무엇을 어떻게 돌릴지를
  **이슈 하나에 자족적으로** 적어요 — 그 기기의 세션이 이슈만 읽고 진행할 수 있어야 해요
  (배경 · 무엇이 어디로 바뀌었는지 · OS 별 실행 명령 · 판정 기준 · 이미 확인한 것).
  첫 적용은 [#645](https://github.com/ensky0/tildaz/issues/645) 예요.
- **검증이 끝나면 그 결과를 PR 본문에 세 OS 로 나눠 적어요.** "macOS 통과" 만 적힌 PR 은 나머지
  둘이 안 된 건지 안 적은 건지 구별이 안 돼요.
- **한 OS 에만 닿는 변경이면 그 OS 만 실기로 보고, 나머지 둘은 `zig build check` (6 타겟) 로
  컴파일만 봐요.** 어느 쪽인지는 *코드로 판별해요* — 위 `# 버그 / 미동작 발견 시` 의 "공유 경로면
  전 환경 잠재" 와 같은 판정이에요.
...
**동작 정책을 정할 때는 관례부터 찾아보고 추천해요 — 우리 `SPEC.md` 는 근거가 아니에요.**
2026-09-05 ([#626](https://github.com/ensky0/tildaz/issues/626)) 에 dead key 조합 중 Enter 가 삼켜지는 것을 결함으로 보고
*"제어 · nav 키는 보내야 한다"* 를 권장안으로 냈는데, 근거가 우리 SPEC 문장 하나였어요. 실제로는 `xkbcommon-compose.h` 가
*"libX11 은 삼킨다"* 고 적고 foot · kitty · GTK 가 전부 삼켜요 — 같은 기기의 foot 로 재 보니 `65 0d` 로 Enter 가 삼켜졌고요.
확인에 5 분 걸렸는데 결과는 정반대였어요. 사용자 지적: *"봐. 네가 권장하는게 일반적인 처리가 아니었잖아. 앞으로 잘 찾아보고
추천해 주길 바래"*. **선택지를 내기 전에** ① 프로토콜 · 라이브러리 문서, ② 대표 구현 두셋의 소스, ③ 가능하면 이 기기에서의
실측 — 셋을 근거로 달아요. 우리 문서와 관례가 어긋나면 **우리 문서가 틀렸을 가능성을 먼저** 봐요.
...
# API 는 쓰기 전에 정의를 읽어요 — 컴파일러에게 묻지 않아요

**표준 라이브러리 · 외부 라이브러리 · 이 저장소의 함수를 쓸 때는 먼저 그 정의나 기존
사용례를 읽고 씁니다.** 기억으로 쓰고 컴파일 에러를 보고 고치는 방식은 쓰지 않아요
(2026-09-11 사용자 지시: *"api 문서나 소스코드를 보고 코드를 써야지, 코드 써서 테스트 해 보고
이게 아닌가 보다 하는건 너무 오래 걸려"*).

두 가지가 나빠요. **느려요** — 한 번 확인하면 끝날 것을 빌드 한 바퀴 (ghostty 를 끼고 있어
수십 초~분) 씩 태워요. 그리고 **컴파일러가 잡아 주는 것만 고치게 돼요** — 시그니처가 우연히
맞으면 틀린 의미로 지나가고, 런타임에만 드러나는 오용 (인자 순서 · 소유권 · 경계 해석) 은
아예 안 걸려요. 위 `# 근본 해결 원칙` 의 *"일단 해보고 시연으로 확인 절대 금지"* 와 같은
규칙을 API 사용에 적용한 것이에요.

**확인하는 법은 대상마다 달라요.**

| 대상 | 어디서 확인 |
|---|---|
| zig 표준 라이브러리 | **레포 안의 기존 사용례를 먼저 grep** 해요 — 0.16 전환을 이미 겪은 코드가 답을 갖고 있어요. 없으면 `$(zig env \| grep lib_dir)` 의 std 소스 |
| ghostty-vt · 의존 패키지 | `zig-pkg/` 아래 실제 소스. tarball 을 풀어 둔 것이라 그 pin 의 진짜 코드예요 |
| 이 저장소의 함수 | 정의로 가서 시그니처와 머리 주석을 읽어요. 주석에 *왜 그렇게 생겼는지* 가 적혀 있는 경우가 많아요 |

**2026-09-11 [#646](https://github.com/ensky0/tildaz/issues/646) 에서 실제로 낭비했어요.**
다섯 번을 컴파일러에게 물었는데 전부 먼저 읽었으면 한 번에 맞았어요.

| 쓴 것 | 실제 | 어디에 답이 있었나 |
|---|---|---|
| `std.posix.getenv` | 0.16 에 없음 | `runtime.zig` 의 `envAlloc` — 그 자리를 대신하려고 만든 함수이고 주석에 이유까지 있었어요 |
| `std.time.Timer` | `Io.Timestamp` 로 합쳐짐 | 같은 파일 주석에 *"0.16 이 Timer 와 Instant 를 Io.Timestamp 하나로 합쳤다"* 라고 적혀 있었어요 |
| `std.meta.intToEnum` | 0.16 에 없음 | std 소스 |
| `log.appendLine(fmt, args)` | `(category, fmt, args)` | **우리 함수**예요. 정의 한 줄만 보면 됐어요 |
| `Terminal.alternateScreen` | `switchScreen(.alternate)` | `zig-pkg/` 의 ghostty 소스 |

같은 회차에 **ghostty 검색 API 는 소스를 먼저 읽어서 한 번에 맞았어요** — `ScreenSearch.init` ·
`tick` · `select` 의 시그니처와 `updateHighlightsFlattened` 의 tag 규칙을 다 읽고 썼고 컴파일
에러가 없었어요. 읽는 비용이 고치는 비용보다 늘 쌉니다.

**예외는 "읽을 것이 없을 때" 뿐이에요.** 문서도 소스도 없는 OS quirk (compositor 동작 · 드라이버
반응) 는 이 문서 곳곳의 실측 절차대로 재는 것이 맞아요 — 그건 try and error 가 아니라 *측정*
이에요. 읽을 수 있는 것을 안 읽고 시도하는 것만 금지예요.
...
- **어느 쪽으로 가는지는 데스크톱마다 갈려요** (2026-09-03 · 같은 4K 화면에서 실측):
  ...
  **compositor 의 논리 화면 · work-area 는 실행 중에도 바뀌어요** — 배율 변경 (`3840 ÷ 1.7 = 2259` vs `÷ 1.6 = 2400`)
  과 **패널의 exclusive zone 변경** (패널 추가 · 제거 · 높이 · 자동 숨김) 둘 다예요. 후자는 우리에게 오는 직접 신호가
  없어요 (다른 client 의 zone 은 프로토콜상 안 보여요) — 관측 가능한 사실은 "받은 configure 크기가 `work-area − 여백`
  과 맞는가" 뿐이라, [#631](https://github.com/ensky0/tildaz/issues/631) 이 그 불변식으로 latch 를 다시 받게 했어요.
  **배율을 바꾼 회차의 판정은 그때 이미 떠 있던 창과 새로 띄운 창을 갈라서 봐요** — 새 창은 그 배율로 latch 하니
  정상이고, 증상은 켜져 있던 창에만 남아요.
...
```powershell
python tool\clusters.py stack2 --cmd $env:TEMP\stack2.txt > $env:TEMP\stack2.cmd   # 화면 (.cmd 래퍼 + UTF-8 본문)
...
macOS 의 `deadkey-check_macos.sh` 에 대응하는 도구 둘이에요 ([#583](https://github.com/ensky0/tildaz/issues/583) A2 · A3 의 Windows 몫).
...
- **PowerShell 은 원소가 하나인 배열을 평탄화해요** — `@(@($Shift, $A))` 는 `@(16, 65)` 가 되어 chord 가 **키 두 개를 따로**
  ...
  **`switch` 의 출력도 같아요** — `$seq = switch ($n) { 2 { @(,$chord) } }` 는 파이프라인을 지나며 풀려 `@(17, 16, 39)` 가
  돼요 (2026-09-03 `split-panes_windows.ps1` — pane 2 회차 5 회가 전부 분할 실패). 블록 안에서 `$seq = ,$chord` 로 **직접 대입**해요.
...
- **`wtype` 은 쓰지 말아요 — 두 가지가 실측으로 걸렸어요.** ① 새 글자가 나올 때마다 keymap 을 다시 올려서 그 사이의 키가
  ...
  `vkbd_linux.py` 는 us keymap 을 연결 직후 한 번 올리고 프로세스가 살아 있는 동안 유지해요.
- **modifier 는 `modifiers` 요청으로 보내야 해요.** 프로토콜이 modifier 상태를 client 책임으로 두고 wlroots 는 가상
  ...
  들어가요. `vkbd_linux.py` 가 mask (Shift 0 · Control 2 · Mod1 3 · Mod4 6 비트) 를 함께 보내요.
- **이 도구로 찾은 결함 — [#347](https://github.com/ensky0/tildaz/issues/347) (2026-09-03).** `wtype` 이 호출마다 가상
  ...
  `fix/347-seat-capability-release` 가 잃으면 `release` · 돌아오면 재생성으로 고쳤고, **`headless-check_linux.sh seat-replug`**
...
- **`-e` 로 띄운 회차는 그 프로세스가 살아 있는지부터 확인해요 — 캡처에는 *다른 창* 이 그 자리에 있을 수 있어요.**
  2026-09-05 에 `-e "sleep 12"` 로 테스트 창을 띄웠는데, `-e` 는 **실행 파일 경로**를 받으므로 그 인자는 실행되지
  않고 창이 0.01 초 만에 닫혔어요 (`[tab] shell exited`). 그런데 캡처에는 같은 자리에 **사용자의 인스턴스 창**이
  있어서, 그것을 제 창으로 알고 쟀어요. 게다가 두 빌드를 그렇게 비교하니 값이 **똑같이** 나와서 (같은 창을 두 번
  잰 것) 그 *일치* 가 "compositor 가 원인" 이라는 틀린 결론의 근거가 됐어요 ([#630](https://github.com/ensky0/tildaz/issues/630) 전말).
  회차마다 이 셋을 봐요 — ① 프로세스가 아직 사는지 (`kill -0`), ② 앱 로그에 `shell exited` 가 없는지,
  ③ 캡처에서 찾은 창이 **내 창인지** (창 안에 회차 표식을 그리는 화면을 쓰면 확실해요 — `bands` 처럼).
  오래 사는 수신자는 스크립트 파일로 둬요: `printf '#!/bin/sh\nwhile :; do sleep 1; done\n' > hold.sh`.
...
- **nested 로 못 가르는 것은 실제 세션에서 재요 — 두 방법이 달라요.** Hyprland 는 D-Bus 의존이 적어 **다른 TTY 에서
  ...
  소켓이 생기므로 agent 가 거기 붙어 `real-session-check_linux.sh hypr-scale` 을 돌릴 수 있어요 (사용자는 화면만 그 VT 에 두면
  ...
  뒤 **그 세션 안의 새 agent 세션**에서 `real-session-check_linux.sh gnome` 을 돌려요. 두 스크립트 모두 세션 배율을 바꾸면 끝에
...
- **Hyprland 0.56 의 설정 파서는 Lua 예요 — `hyprctl keyword` 가 `keyword can't work with non-legacy parsers. Use eval.` 로
  ...
  keyword 가 그대로 먹어요 — `real-session-check_linux.sh` 의 `set_scale` 이 응답을 보고 갈라요. 첫 라운드는 이걸 몰라 세 회차가
...
## ⑥ atlas · 렌더 경로를 바꿨을 때 — 판을 맞대 픽셀로 (`render-ab-shot`)
