...
| Session core | Yes | `src/session_core.zig` | Tabs — each a `TabGroup` of panes (`Tab`) arranged by a `pane_layout.Tree` (split / close / focus / resize operations land here — [#483](https://github.com/ensky0/tildaz/issues/483); the tree stays one leaf on macOS and Windows until their hosts are wired) — active index, scrollback, VT draining across all panes, PTY queues, and each pane's ghostty `RenderState` snapshot (panes outside the active tab release theirs) |
| Tab behavior | Yes | `src/tab_actions.zig`, `src/tab_interaction.zig`, `src/tab_layout.zig`, `src/pane_layout.zig` | Tab switching, close paths, drag, hit testing. `pane_layout.zig` holds the split-pane tree and per-pane grid geometry (hit test, neighbor search) as pure functions — [#483](https://github.com/ensky0/tildaz/issues/483); wired into the Linux host (step 4b), macOS and Windows follow at step 5 |
...
| Split panes | [#483](https://github.com/ensky0/tildaz/issues/483) | Slow track. Steps 1–3 of 6 landed with zero visible change (pixel A/B identical on all three platforms): `src/pane_layout.zig` (pure split tree + per-pane grid, hit testing, neighbor search), the three renderers split into `drawPane(PaneDraw)` / `endFrame`, `SessionCore` tabs became `TabGroup`s over that tree. Step 4a added the `[keys]` pane actions and `SessionCore` operations on every platform; steps 4b–4c wired the Linux host (split / focus / resize / equalize / zoom, gray separators + amber focus line, click-to-focus, separator drag, `…` menu items, `+` Alt-click) and step 5 did the same for macOS and Windows (Windows still awaiting hands-on verification). Next: IME / drain / per-pane scrollbar (step 6) |
