...
#   perf 종료 덤프의 `render calls` 로 본다 (`tool/clusters.py mini`).
...
if [ ! -x "$CAP" ] || [ "$HERE/color-capture_macos.m" -nt "$CAP" ]; then
  mkdir -p "$(dirname "$CAP")"
  clang -fobjc-arc -framework Cocoa -framework ScreenCaptureKit -framework ImageIO \
        -framework UniformTypeIdentifiers -o "$CAP" "$HERE/color-capture_macos.m" || exit 1
fi
