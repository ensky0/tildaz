...
#
#   tool/real-session-check_linux.sh hypr-scale [1.25 ...]   # 다른 TTY 에 뜬 실제 Hyprland 에 붙어 배율별 띠 화면 전이 행 (foot 대조 포함)
#   tool/real-session-check_linux.sh hypr-height 1.25 60 50 40   # 같은 배율 안에서 논리 높이만 바꿔 원인이 우리 산술인지 가린다 (#619 실험 ③)
#   tool/real-session-check_linux.sh no-layer-shell           # 실제 GNOME / Cinnamon (Wayland) 세션 안에서: fractional-scale 지원 통보 · 앱의 scale 소스 · #577 launcher fatal 다이얼로그
#
...
measure() {   # $1 tag  $2 grim 출력 이름
    ...
    python3 "$ROOT/tool/bands-check.py" $WORK/$1.png --locate | sed 's/^/    /'
}
...
cmd_no_layer_shell() {
    ...
    python3 "$ROOT/tool/portal-screenshot_linux.py" "$WORK/dialog.png" --timeout 90 2>&1 | sed 's/^/    /'
...
#
#   tool/real-session-check_linux.sh hypr-height 1.25 60 50 40
#
