...
같은 nested schema, default 만 OS-specific. Schema validation 은 *single source of truth* 패턴 — `DEFAULT_CONFIG_JSON` 한 string 이 schema (key set + nested 구조 + value type) 의 ground truth, 새 사용자에게 그대로 createDefault 로 작성 + 기존 config 의 `validateStructure` 검증에 활용. 새 필드 추가 시 *그 한 곳만* update 하면 required / unknown / type 검사 자동 sync.
...
| `font.family` | string / array | `["Cascadia Mono", "Malgun Gothic", "Segoe UI Symbol"]` | `["Menlo"]` (단일 — system auto fallback 충분) | ✅ | ✅ |
| `font.size` | integer 8..72 | 19 | 15 (Apple Terminal/iTerm2 컨벤션 + retina) | ✅ | ✅ |
| `font.line_height` | float | 0.95 | 1.1 (Apple HIG) | ✅ | ✅ |
| `font.cell_width` | float | 1.1 | 1.0 (Menlo metric 자연) | ✅ | ✅ |
...
> **font.family glyph fallback chain** (Windows DWriteFontContext 와 동등): codepoint 별로 chain 순회 → 글리프 가진 첫 폰트 사용. 모든 명시 폰트가 system 에 register 되어 있어야 함 (한 개라도 없으면 fatal `Font not found: "Foo"`). macOS 는 chain 에 없는 codepoint 는 system auto fallback (`CTFontCreateForString` — Apple Color Emoji / Apple SD Gothic Neo 등) 으로 보충 — Windows 는 chain 에 모든 script 명시 필요. macOS substitute font 회피 위해 `CTFontCopyFamilyName` 으로 *실제 family name* 검증.
...
| 컬러 emoji + grapheme cluster shaping | 🟡 | [#134](https://github.com/ensky0/tildaz/issues/134) | macOS #132 (commit `48035eb` + `602dfe1`) 동등성. 현재 Windows 는 (a) 컬러 글리프 atlas/셰이더 path 부재 → emoji monochrome outline, (b) DirectWrite shaping 부재 → skin tone modifier / ZWJ 시퀀스 분리 표시, (c) `Segoe UI Emoji` 가 chain 에 있어도 무용 (atlas path 가 컬러 안 받음). 2026-05-03 Windows 시연에서 발견. About / 다른 path 의 텍스트 렌더링은 정상. |
